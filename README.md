...
![Source Control: GitHub and Azure DevOps](https://img.shields.io/badge/Source%20Control-GitHub%20%7C%20Azure%20DevOps-181717?logo=github)

## Description

**ContosoDevExp** is an Azure Developer CLI (`azd`) accelerator that automates
end-to-end provisioning of a Microsoft Dev Box environment on Azure. It deploys
a Dev Center with projects, Dev Box pools, network connections, security
resources, and a Log Analytics workspace, enabling platform engineering teams to
deliver standardized cloud-hosted developer workstations at scale.

The accelerator eliminates the manual, error-prone work of configuring Microsoft
Dev Box infrastructure. By expressing all Dev Center settings, project
definitions, pool configurations, and network connections as declarative YAML
files and Bicep templates, it removes configuration drift and reduces
environment provisioning time from hours to minutes.

The project relies on **Bicep** for infrastructure as code, the Azure Developer
...
to teams on Windows, Linux, and macOS.
...
- 🏢 **Automated Dev Center provisioning** — deploys a Dev Center with
  system-assigned managed identity, Azure Monitor agent, and catalog item sync
  enabled
- 📁 **Multi-project support** — provisions one or more Dev Center projects from
  YAML, each with isolated catalogs, pools, and environment types
- 🖥️ **Role-specific Dev Box pools** — configures pools per engineering role
  (for example, backend and frontend) with distinct VM SKUs and image
  definitions
- 🌐 **Flexible network connectivity** — supports Microsoft-hosted networks and
  custom Azure Virtual Networks with Microsoft Entra ID join
- 🔑 **Secure secrets management** — stores source control tokens in Azure Key
  Vault with RBAC authorization, soft-delete protection, and purge protection
  enabled
- 📊 **Integrated observability** — streams diagnostics and metrics from all
  resources to a central Log Analytics workspace
- 📚 **Catalog synchronization** — connects GitHub or Azure DevOps repositories
  as Dev Center catalogs with scheduled sync for image and environment
  definitions
- ⚙️ **Configuration-as-code** — all Dev Center, security, and resource
  organization settings are YAML files in `infra/settings/`, versioned alongside
  the infrastructure
...
- 🤖 **Cross-platform automation** — `azd` preprovision hooks invoke `setUp.sh`
  on Linux and macOS, and `setUp.ps1` on Windows

## Architecture

**ContosoDevExp** provisions a Microsoft Dev Box platform on Azure, enabling
developers to self-serve cloud-hosted workstations. A Platform Admin deploys the
accelerator using `azd up`, a Dev Manager configures Dev Center projects and
pools, and a Developer provisions and uses Dev Box instances through the Dev Box
portal. The Dev Center acts as the central hub, orchestrating project and pool
management, catalog synchronization with source control, secret retrieval from
Key Vault, diagnostic streaming to Log Analytics, and Virtual Network
connectivity with Entra ID join.
...
%% ── External Systems ────────────────────────────────────────────────────────
SourceControl(["🐙 GitHub / Azure DevOps"])
...
DevManager -->|"configures projects and pools"| Project
Developer -->|"provisions Dev Box"| Pool

%% ── Internal Synchronous Flows ───────────────────────────────────────────────
DevCenter -->|"creates and manages"| Project
...
classDef datastoreStyle fill:#fefbf4,stroke:#f9e2ae,color:#242424

class PlatformAdmin,DevManager,Developer actorStyle
class SourceControl,EntraID externalStyle
class DevCenter,Project,Pool,Catalog,EnvType,NetConn serviceStyle
class KeyVault,LogAnalytics,VNet datastoreStyle

%% ── Subgraph Styles ──────────────────────────────────────────────────────────
style AzureSub fill:#ebf3fc,stroke:#0f6cbd,color:#242424
style WorkloadRG fill:#fafafa,stroke:#e0e0e0,color:#242424
style SupportRG fill:#fefbf4,stroke:#f9e2ae,color:#242424
```
...
| Gate                           | Result | Notes                                                                                                                                                                                                                                                                                                                                |
| ------------------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Gate 0 — Architecture Intent   | PASSED | Three human actors present. Developer → Pool → VNet traces to a provisioned cloud workstation. The diagram shows actors interacting with a running system, not a build pipeline or infrastructure org chart.                                                                                                                         |
| Gate 1 — Constraint Compliance | PASSED | All 19 positive constraints satisfied. `align` and `description` are excluded from the config block because they are not valid Mermaid config properties (Constraint 15 requires valid syntax only). Subgraphs are styled with `style` directives because `classDef`/`class` assignments do not apply to subgraph blocks in Mermaid. |
| Gate 2 — Evaluation Criteria   | PASSED | All 8 criteria pass: actor presence, goal traceability, component coverage (14 nodes), interaction labeling on every edge, shape and style compliance, Fluent UI v9 color tokens, node limit satisfied. Mermaid syntax validity has been verified against the v11 specification; machine rendering was not available.                |
| Gate 3 — Styling Compliance    | PASSED | Config block contains all valid theming variables from the output template. Every hex value traces to a named Fluent UI v9 semantic token. `align` and `description` are omitted because they have no corresponding Mermaid property.                                                                                                |
...
| [Azure Developer CLI (azd)](https://learn.microsoft.com/en-us/azure/developer/azure-developer-cli/)        | Deployment tooling     | Manages environment lifecycle: init, provision, deploy, teardown                  |
| YAML                                                                                                       | Configuration          | Parameterizes Dev Center projects, pools, catalogs, and resource organization     |
| PowerShell                                                                                                 | Automation             | Windows preprovision scripting via `setUp.ps1`                                    |
| Bash                                                                                                       | Automation             | Linux and macOS preprovision scripting via `setUp.sh`                             |
...
| [Azure Key Vault](https://learn.microsoft.com/en-us/azure/key-vault/general/basic-concepts)                | Azure PaaS             | Stores source control tokens with RBAC authorization and soft-delete protection   |
...
| [Azure Log Analytics](https://learn.microsoft.com/en-us/azure/azure-monitor/logs/log-analytics-overview)   | Azure Monitoring       | Centralizes diagnostics, metrics, and activity logs for all resources             |
| [Microsoft Entra ID](https://learn.microsoft.com/en-us/entra/fundamentals/whatis)                          | Identity               | Provides Azure AD join for Dev Box domain authentication                          |
| [Hugo (extended)](https://gohugo.io/)                                                                      | Documentation          | Generates the project documentation site                                          |
| Node.js / npm                                                                                              | Build tooling          | Manages Hugo build dependencies and the documentation pipeline                    |
...
| Bash or PowerShell                                                                                             | —               | Linux and macOS: Bash; Windows: PowerShell 5.1 or later |
| [jq](https://jqlang.github.io/jq/)                                                                             | 1.6+            | Required by `setUp.sh` on Linux and macOS               |
...
3. Authenticate the GitHub CLI (GitHub only):
...
When prompted, provide the environment name (2–10 characters), the Azure region,
and the personal access token for your source control platform.
...
> [!NOTE] `azd up` automatically executes `setUp.sh` on Linux and macOS, or
> `setUp.ps1` on Windows, as a preprovision hook before any Bicep templates are
> deployed.

## Configuration

All configuration is stored in `infra/settings/` as YAML files. Edit these files
before running `azd up` to customize the deployment for your organization.

| Option                    | File                                                      | Default           | Description                                                   |
| ------------------------- | --------------------------------------------------------- | ----------------- | ------------------------------------------------------------- |
| `AZURE_ENV_NAME`          | `infra/main.parameters.json`                              | _(required)_      | Environment name used in resource naming; 2–10 characters     |
| `AZURE_LOCATION`          | `infra/main.parameters.json`                              | _(required)_      | Azure region for all resources, for example `eastus`          |
...
| Key Vault base name       | `infra/settings/security/security.yaml`                   | `contoso`         | Base name of the Key Vault resource                           |
...
> [!IMPORTANT] The `KEY_VAULT_SECRET` must be a personal access token with
> `repo` scope for GitHub, or `Code (Read)` scope for Azure DevOps, to enable
> catalog synchronization.

## Deployment

1. Install all [prerequisites](#prerequisites) and authenticate to Azure.

2. Set the source control platform environment variable:
...
3. Review and customize the YAML files in `infra/settings/` for your
   organization.
...
5. When prompted, enter the environment name, select the Azure subscription,
   choose the Azure region, and provide the Key Vault secret value.

6. The preprovision hook (`setUp.sh` or `setUp.ps1`) runs automatically and
   configures the `azd` environment before Bicep templates are deployed at the
   subscription scope.

7. Retrieve the deployed resource details after provisioning completes:
...
> any custom configuration before executing this command.

## Usage

### Access the Dev Box Portal

Developers access their Dev Boxes at the
[Microsoft Dev Box portal](https://devbox.microsoft.com) or through the Azure
portal under the provisioned Dev Center.

### List Available Dev Box Types
...
> [!CAUTION] `azd down` permanently deletes all Azure resources provisioned by
> this accelerator, including the Dev Center, Key Vault, and Log Analytics
> workspace. This action cannot be undone.

## Contributing

Report bugs and request features on the
[GitHub Issues page](https://github.com/Evilazaro/DevExp-DevBox/issues).

To submit a pull request:

1. Fork the repository on GitHub.
2. Create a branch from `main` for your change.
3. Test your changes by running `azd up` against a dedicated Azure environment.
4. Update the relevant YAML schema files in `infra/settings/` if your change
   adds new configuration options.
5. Open a pull request against `main` with a clear description of the change and
   its motivation.

Follow the existing Bicep type annotation style and keep YAML configuration
files aligned with their `.schema.json` counterparts in `infra/settings/`.
...
Developer who provisions and uses Dev Box instances. Internally, the Dev Center
acts as the central hub, orchestrating project and pool management, catalog
synchronization with GitHub or Azure DevOps, secret retrieval from Azure Key
Vault, diagnostic streaming to Log Analytics, and network connectivity through
Azure Virtual Networks with Entra ID join.
