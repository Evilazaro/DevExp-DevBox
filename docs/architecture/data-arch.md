...
> **Gap**: No formal versioned data contracts or AsyncAPI/OpenAPI specifications
...
| Data Domain                | Component                           | Maturity       | Evidence                                                                                     |
| -------------------------- | ----------------------------------- | -------------- | -------------------------------------------------------------------------------------------- |
| Schema Governance          | JSON Schema validation (3 schemas)  | 🟢 **Level 4** | `additionalProperties: false`, required fields, pattern validation across all 3 schema files |
| Secret Management          | Azure Key Vault (RBAC + protection) | 🟢 **Level 4** | `enableRbacAuthorization`, `enablePurgeProtection`, `enableSoftDelete`, diagnostic settings  |
| Audit & Monitoring         | Log Analytics + Diagnostic Settings | 🟡 **Level 3** | `allLogs` configured; no alerting rules or retention policies customized                     |
| Tag-Based Classification   | 8-field tag schema                  | 🟡 **Level 3** | Uniformly applied; no automated Policy enforcement in Azure; no DLP tooling                  |
| Configuration Data Quality | YAML + Schema validation            | 🟡 **Level 3** | Design-time validation only; no runtime data quality checks or monitoring dashboards         |
| Data Flow Traceability     | `loadYamlContent()` pipeline        | 🟡 **Level 3** | Implicit traceability via Bicep source references; no automated lineage graph or catalog     |
| Data Contracts             | JSON Schema `$schema` references    | 🟠 **Level 2** | Implicit contracts only; no versioned interface specifications or formal contract testing    |
| Master Data                | RBAC GUIDs / AAD Group IDs          | 🔴 **Level 1** | Hard-coded literals in YAML; no centralized reference data registry                          |
...
| Name                                  | Description                                                                                | Sub-Type           | Classification | Format / Schema                   | Owner       | Access Control          | Retention            | Dependencies                                         | Source Reference                                                       |
| ------------------------------------- | ------------------------------------------------------------------------------------------ | ------------------ | -------------- | --------------------------------- | ----------- | ----------------------- | -------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------- |
| `DevCenterConfig`                     | Top-level DevCenter configuration type; defines name, identity, feature statuses, and tags | Configuration Type | Internal       | Bicep `type` declaration          | DevExP Team | Bicep compile-time only | Repository lifecycle | `Identity`, `Tags`, `Catalog[]`, `EnvironmentType[]` | `src/workload/core/devCenter.bicep:31-51`                              |
| `Identity`                            | Managed identity configuration type; defines identity type and role assignment structure   | Configuration Type | Internal       | Bicep `type` declaration          | DevExP Team | Bicep compile-time only | Repository lifecycle | `RoleAssignment`, `OrgRoleType`                      | `src/workload/core/devCenter.bicep:52-62`                              |
| `AzureRBACRole`                       | Azure RBAC role definition type; maps role GUID to name and scope                          | Reference Type     | Internal       | Bicep `type` declaration          | DevExP Team | Bicep compile-time only | Repository lifecycle | `Identity`                                           | `src/workload/core/devCenter.bicep:75-85`                              |
| `OrgRoleType`                         | Organization role type mapping Azure AD groups to RBAC roles                               | Configuration Type | Restricted     | Bicep `type` declaration          | DevExP Team | Bicep compile-time only | Repository lifecycle | `AzureRBACRole`                                      | `src/workload/core/devCenter.bicep:87-97`                              |
| `Catalog`                             | DevCenter catalog type; repository reference for environment/image definitions             | Configuration Type | Internal       | Bicep `type` declaration          | DevExP Team | Bicep compile-time only | Repository lifecycle | `DevCenterConfig`                                    | `src/workload/core/catalog.bicep:9-25`                                 |
| `EnvironmentType`                     | Environment type configuration; defines deployment environment name and target             | Configuration Type | Internal       | Bicep `type` declaration          | DevExP Team | Bicep compile-time only | Repository lifecycle | `DevCenterConfig`, `Project`                         | `src/workload/core/environmentType.bicep:7-10`                         |
| `PoolConfig` / `DevBoxPool`           | Dev Box pool configuration type; maps image definition and VM SKU                          | Configuration Type | Internal       | Bicep `type` declaration          | DevExP Team | Bicep compile-time only | Repository lifecycle | `ProjectCatalog`, `NetworkSettings`                  | `src/workload/project/projectPool.bicep`                               |
| `KeyVaultSettings` / `KeyVaultConfig` | Key Vault configuration type; defines all security and soft-delete settings                | Configuration Type | Confidential   | Bicep `type` declaration          | DevExP Team | Bicep `@secure()`       | Repository lifecycle | `Tags`                                               | `src/security/keyVault.bicep:9-30`                                     |
| `Tags`                                | Universal governance tag type; 8-field wildcard schema                                     | Governance Type    | Internal       | Bicep `type Tags = { *: string }` | DevExP Team | All Bicep modules       | Repository lifecycle | All resource types                                   | Multiple files (e.g., `src/security/keyVault.bicep:32-34`)             |
| `NetworkSettings` / `ProjectNetwork`  | Network configuration type; VNet creation, subnets, address prefixes                       | Configuration Type | Internal       | Bicep `type` declaration          | DevExP Team | Bicep compile-time only | Repository lifecycle | `Project`                                            | `src/connectivity/vnet.bicep:13-31`                                    |
| `LandingZone`                         | Resource organization type; resource group creation flags, names, tags                     | Configuration Type | Internal       | Bicep `type` declaration          | DevExP Team | Bicep compile-time only | Repository lifecycle | `Tags`                                               | `src/workload/workload.bicep:18-28`                                    |
| `ResourceGroupConfig`                 | Resource group configuration type; name, create flag, description, tags                    | Configuration Type | Internal       | JSON Schema `$defs/resourceGroup` | DevExP Team | Bicep compile-time only | Repository lifecycle | `Tags`                                               | `infra/settings/resourceOrganization/azureResources.schema.json:20-65` |

### 5.2 Data Models

| Name                          | Description                                                                                                                | Sub-Type           | Classification | Format / Schema                                               | Owner       | Access Control                                  | Retention            | Dependencies                 | Source Reference                                               |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------ | -------------- | ------------------------------------------------------------- | ----------- | ----------------------------------------------- | -------------------- | ---------------------------- | -------------------------------------------------------------- |
| DevCenter Configuration Model | Operational YAML model for Azure DevCenter; defines name, identity, RBAC, catalogs, environment types, projects, pools     | YAML Configuration | Internal       | YAML 1.2 / JSON Schema 2020-12 (`devcenter.schema.json`)      | DevExP Team | Repository read access                          | Repository lifecycle | `devcenter.schema.json`      | `infra/settings/workload/devcenter.yaml:1-190`                 |
| Key Vault Security Model      | Operational YAML model for Azure Key Vault; defines name, secret name, security flags, soft-delete, tags                   | YAML Configuration | Confidential   | YAML 1.2 / JSON Schema 2020-12 (`security.schema.json`)       | DevExP Team | Repository read access (secret values excluded) | Repository lifecycle | `security.schema.json`       | `infra/settings/security/security.yaml:1-43`                   |
| Resource Organization Model   | Operational YAML model for Azure resource groups; defines workload, security, monitoring groups with create flags and tags | YAML Configuration | Internal       | YAML 1.2 / JSON Schema 2020-12 (`azureResources.schema.json`) | DevExP Team | Repository read access                          | Repository lifecycle | `azureResources.schema.json` | `infra/settings/resourceOrganization/azureResources.yaml:1-80` |
| Deployment Parameters Model   | ARM deployment parameters binding `azd` environment variables to Bicep parameter values                                    | Parameters File    | Internal       | JSON (ARM Parameters schema v2019-04-01)                      | DevExP Team | Repository read access                          | Repository lifecycle | `main.bicep`                 | `infra/main.parameters.json:1-14`                              |

### 5.3 Data Stores

| Name                                              | Description                                                                                                                      | Sub-Type                   | Classification | Format / Schema                            | Owner       | Access Control                                                                                | Retention                                                   | Dependencies                                                              | Source Reference                          |
| ------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------- | -------------- | ------------------------------------------ | ----------- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------------- |
| Azure Key Vault (`contoso-<unique>-kv`)           | Secure secrets store; holds GitHub Access Token (`gha-token`) for private DevCenter catalog authentication; SKU: Standard        | Azure Managed Secret Store | Confidential   | Azure Key Vault REST API v7.x / JSON       | DevExP Team | RBAC (`Key Vault Secrets User`, `Key Vault Secrets Officer`); `enableRbacAuthorization: true` | 7-day soft delete; indefinite with purge protection enabled | Log Analytics (diagnostic target), DevCenter (secret consumer)            | `src/security/keyVault.bicep:41-65`       |
| Log Analytics Workspace (`logAnalytics-<unique>`) | Centralized telemetry and audit data store; captures Key Vault allLogs, AllMetrics, DevCenter diagnostic streams; SKU: PerGB2018 | Azure Managed Log Store    | Internal       | Azure Monitor Log Analytics REST API / KQL | DevExP Team | Azure RBAC (workspace access); workspace key for agent integration                            | PerGB2018 default (30 days free; customizable)              | Key Vault (log source), DevCenter (metric source), AzureActivity solution | `src/management/logAnalytics.bicep:33-68` |

### 5.4 Data Flows

| Name                            | Description                                                                                                                                                                    | Sub-Type           | Classification | Format / Schema                       | Owner       | Access Control                        | Retention                                | Dependencies                                             | Source Reference                                                                             |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------ | -------------- | ------------------------------------- | ----------- | ------------------------------------- | ---------------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| YAML → Bicep Configuration Load | `loadYamlContent()` transforms YAML config models into Bicep parameter objects; executes at Bicep compilation/deployment time                                                  | Configuration Load | Internal       | YAML → Bicep typed object             | DevExP Team | Deployment pipeline credentials       | No persistence (compile-time)            | `devcenter.yaml`, `security.yaml`, `azureResources.yaml` | `infra/main.bicep:33-35`, `src/security/security.bicep:18`, `src/workload/workload.bicep:43` |
| Secret Injection Flow           | `secretValue` (GitHub PAT) flows from `azd` environment variable → `main.bicep` parameter → `security.bicep` → `secret.bicep` → Azure Key Vault secret object                  | Secret Ingestion   | Confidential   | `@secure()` string → Key Vault secret | DevExP Team | `@secure()` parameter; Key Vault RBAC | Key Vault retention policy               | `main.bicep` (`secretValue` param), Key Vault            | `src/security/secret.bicep:1-25`                                                             |
| Secret Reference Flow           | Key Vault `secretUri` output flows as `secretIdentifier` parameter from `security.bicep` → `workload.bicep` → `devCenter.bicep` → `catalog.bicep` for private GitHub repo auth | Secret Reference   | Confidential   | Key Vault URI string; `@secure()`     | DevExP Team | `@secure()` parameter chain           | No persistence (deployment reference)    | Azure Key Vault, DevCenter catalogs                      | `src/workload/workload.bicep:57`, `src/workload/core/catalog.bicep:51-64`                    |
| Monitoring Data Flow            | Azure Key Vault audit logs + AllMetrics → Diagnostic Settings → Log Analytics Workspace; DevCenter Azure Monitor agent metrics → Log Analytics                                 | Telemetry Stream   | Internal       | Azure Diagnostics JSON logs           | DevExP Team | Log Analytics workspace access        | Log Analytics workspace retention policy | Key Vault, DevCenter, Log Analytics                      | `src/security/secret.bicep:29-46`, `src/management/logAnalytics.bicep:43-60`                 |

### 5.5 Data Services

| Name                                 | Description                                                                                                                        | Sub-Type        | Classification | Format / Schema                    | Owner       | Access Control                 | Retention           | Dependencies                       | Source Reference                            |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | --------------- | -------------- | ---------------------------------- | ----------- | ------------------------------ | ------------------- | ---------------------------------- | ------------------------------------------- |
| Azure Monitor Agent                  | Telemetry collection service installed on DevCenter Dev Box VMs; controlled by `installAzureMonitorAgentEnableStatus` feature flag | Telemetry Agent | Internal       | Azure Monitor Agent (AMA) protocol | DevExP Team | DevCenter managed identity     | Workspace retention | Log Analytics Workspace, DevCenter | `infra/settings/workload/devcenter.yaml:22` |
| Log Analytics AzureActivity Solution | OMSGallery `AzureActivity` solution deployed alongside Log Analytics Workspace for activity log correlation and analysis           | Log Solution    | Internal       | Azure Activity Log schema (JSON)   | DevExP Team | Log Analytics workspace access | Workspace retention | Log Analytics Workspace            | `src/management/logAnalytics.bicep:47-58`   |

### 5.6 Data Governance

| Name                                  | Description                                                                                                                                                                                       | Sub-Type               | Classification | Format / Schema                      | Owner       | Access Control                      | Retention                                               | Dependencies                                                                  | Source Reference                                                                                                              |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | -------------- | ------------------------------------ | ----------- | ----------------------------------- | ------------------------------------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Tag-Based Governance Schema           | 8-field tag standard (`environment`, `division`, `team`, `project`, `costCenter`, `owner`, `landingZone`, `resources`) applied to all Azure resources; defined in all 3 schema files              | Governance Policy      | Internal       | JSON Schema `tags` object definition | DevExP Team | Architecture governance             | Repository lifecycle                                    | All resource configurations                                                   | `infra/settings/security/security.schema.json:19-90`, `infra/settings/resourceOrganization/azureResources.schema.json:45-120` |
| Azure RBAC Authorization Policy       | Enforces role-based access on Key Vault (`enableRbacAuthorization: true`); assigns `Key Vault Secrets User` and `Key Vault Secrets Officer` to DevCenter managed identity at resource group scope | Access Policy          | Confidential   | Azure RBAC role assignment JSON      | DevExP Team | Azure subscription RBAC             | Azure Resource lifecycle                                | Key Vault, DevCenter identity                                                 | `infra/settings/workload/devcenter.yaml:42-47`                                                                                |
| JSON Schema Strict Validation         | `additionalProperties: false` on all 3 JSON Schema files prevents undocumented fields in configuration YAML; enforces data contract integrity                                                     | Data Quality Policy    | Internal       | JSON Schema 2020-12                  | DevExP Team | Repository governance               | Repository lifecycle                                    | `devcenter.schema.json`, `security.schema.json`, `azureResources.schema.json` | `infra/settings/workload/devcenter.schema.json:8`, `infra/settings/security/security.schema.json:7`                           |
| Soft Delete + Purge Protection Policy | `enableSoftDelete: true`, `softDeleteRetentionInDays: 7`, `enablePurgeProtection: true` prevent permanent secret loss; immutable once purge protection is enabled                                 | Data Protection Policy | Confidential   | Azure Key Vault security properties  | DevExP Team | Key Vault RBAC + Azure subscription | 7-day recovery window; indefinite with purge protection | Key Vault                                                                     | `infra/settings/security/security.yaml:21-25`                                                                                 |

### 5.7 Data Quality Rules

| Name                        | Description                                                                                                                                  | Sub-Type      | Classification | Format / Schema                                   | Owner       | Access Control   | Retention            | Dependencies                                         | Source Reference                                                       |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | -------------- | ------------------------------------------------- | ----------- | ---------------- | -------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------- |
| GUID Format Validation      | `pattern: "^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-...$"` on all role definition IDs and Azure AD group IDs in `devcenter.schema.json`                | Format Rule   | Internal       | JSON Schema `pattern` keyword                     | DevExP Team | Schema validator | Repository lifecycle | `devcenter.schema.json`                              | `infra/settings/workload/devcenter.schema.json:14-21`                  |
| Key Vault Name Pattern Rule | `pattern: "^[a-zA-Z0-9-]{3,24}$"` + `minLength: 3`, `maxLength: 24` enforce Azure naming conventions for Key Vault names                     | Naming Rule   | Internal       | JSON Schema `pattern` + `minLength` + `maxLength` | DevExP Team | Schema validator | Repository lifecycle | `security.schema.json`                               | `infra/settings/security/security.schema.json:45-52`                   |
| Environment Enum Constraint | `enum: ["dev","test","staging","prod"]` on `environment` tag field across all 3 schema files; prevents invalid environment tags              | Enum Rule     | Internal       | JSON Schema `enum` keyword                        | DevExP Team | Schema validator | Repository lifecycle | All 3 schema files                                   | `infra/settings/security/security.schema.json:25-35`                   |
| Required Field Enforcement  | `required: ["name"]` (devcenter), `required: ["create","keyVault"]` (security), `required: ["workload","security","monitoring"]` (resources) | Presence Rule | Internal       | JSON Schema `required` keyword                    | DevExP Team | Schema validator | Repository lifecycle | All 3 schema files                                   | All 3 schema files (line 8 region)                                     |
| Secret Name Pattern Rule    | `pattern: "^[a-zA-Z0-9-]{1,127}$"`, `minLength: 1`, `maxLength: 127` on Key Vault secret names                                               | Naming Rule   | Confidential   | JSON Schema `pattern` + length constraints        | DevExP Team | Schema validator | Repository lifecycle | `security.schema.json`                               | `infra/settings/security/security.schema.json:57-65`                   |
| Tag Value Length Rule       | `maxLength: 256` on all tag values via `additionalProperties` in both `security.schema.json` and `azureResources.schema.json`                | Length Rule   | Internal       | JSON Schema `maxLength` + `additionalProperties`  | DevExP Team | Schema validator | Repository lifecycle | `security.schema.json`, `azureResources.schema.json` | `infra/settings/security/security.schema.json:15-18`                   |
| Resource Group Name Pattern | `pattern: "^[a-zA-Z0-9._-]+$"`, `minLength: 1`, `maxLength: 90` in `azureResources.schema.json`                                              | Naming Rule   | Internal       | JSON Schema `pattern` + length constraints        | DevExP Team | Schema validator | Repository lifecycle | `azureResources.schema.json`                         | `infra/settings/resourceOrganization/azureResources.schema.json:25-35` |
...
| Name                      | Description                                                                                                                                                          | Sub-Type           | Classification | Format / Schema      | Owner       | Access Control         | Retention                | Dependencies                             | Source Reference                                                                          |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | -------------- | -------------------- | ----------- | ---------------------- | ------------------------ | ---------------------------------------- | ----------------------------------------------------------------------------------------- |
| Azure RBAC Role GUIDs     | Hard-coded Azure built-in role definition GUIDs (e.g., `b24988ac-6180-42a0-ab88-20f7382dd24c` = Contributor) embedded as literals in `devcenter.yaml`                | Implicit Reference | Internal       | UUID string literals | DevExP Team | Repository read access | Azure lifetime           | `devcenter.yaml`, `keyVaultAccess.bicep` | `infra/settings/workload/devcenter.yaml:32-47`, `src/identity/keyVaultAccess.bicep:14`    |
| Azure AD Group Object IDs | Azure AD group GUIDs for Dev Manager team (`54fd94a1-e116-4bc8-8238-caae9d72bd12`) and eShop Engineers (`b9968440-0caf-40d8-ac36-52f159730eb7`) embedded as literals | Implicit Reference | Restricted     | UUID string literals | DevExP Team | Repository read access | Azure AD tenant lifetime | `devcenter.yaml`                         | `infra/settings/workload/devcenter.yaml:50`, `infra/settings/workload/devcenter.yaml:115` |
...
| Name                                             | Description                                                                                                                                                                                                                       | Sub-Type               | Classification | Format / Schema                             | Owner       | Access Control            | Retention      | Dependencies                                             | Source Reference                                                                             |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | -------------- | ------------------------------------------- | ----------- | ------------------------- | -------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| YAML-to-Bicep Transformation (`loadYamlContent`) | Built-in Bicep compiler function that deserializes YAML to typed Bicep parameter objects at deployment time; no runtime component                                                                                                 | Compile-Time Transform | Internal       | YAML → Bicep typed object (JSON-compatible) | DevExP Team | Deployment pipeline       | No persistence | `devcenter.yaml`, `security.yaml`, `azureResources.yaml` | `infra/main.bicep:33-35`, `src/security/security.bicep:18`, `src/workload/workload.bicep:43` |
| Resource Naming Transform                        | `'${landingZones.security.name}-${resourceNameSuffix}'` pattern in `main.bicep` computes dynamic resource group names from environment + location context; also includes conditional logic for shared vs separate resource groups | String Transform       | Internal       | Bicep string interpolation                  | DevExP Team | Deployment pipeline       | No persistence | `azureResources.yaml`, deployment parameters             | `infra/main.bicep:36-56`                                                                     |
| Architecture Document Transform                  | `scripts/transform-bdat.ps1` PowerShell script transforms BDAT architecture markdown by removing source/confidence metadata columns, adding emoji headings, and reformatting 5-column tables to 2-column format                   | Document Transform     | Internal       | PowerShell 5.1+; Markdown text              | DevExP Team | Local developer execution | No persistence | Architecture markdown documents                          | `scripts/transform-bdat.ps1:1-80`                                                            |
...
| Name                                  | Description                                                                                                                                                                                                                | Sub-Type            | Classification | Format / Schema                        | Owner       | Access Control                 | Retention                | Dependencies                          | Source Reference                                                                           |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | -------------- | -------------------------------------- | ----------- | ------------------------------ | ------------------------ | ------------------------------------- | ------------------------------------------------------------------------------------------ |
| Bicep `@secure()` Parameter Guard     | `@secure()` decorator applied to `secretValue` and `secretIdentifier` parameters prevents secret values from being logged in Azure deployment history, ARM deployment outputs, or `azd` logs                               | Deployment Security | Confidential   | Bicep `@secure()` decorator            | DevExP Team | Deployment pipeline            | No persistence           | All modules receiving secrets         | `infra/main.bicep:11-12`, `src/security/secret.bicep:4,7`                                  |
| Key Vault RBAC Authorization          | `enableRbacAuthorization: true` enforces Azure RBAC (not legacy Access Policies) for all Key Vault operations; DevCenter managed identity assigned `Key Vault Secrets User` and `Key Vault Secrets Officer` roles          | Access Control      | Confidential   | Azure RBAC / ARM role assignment       | DevExP Team | Key Vault + RBAC admin         | Azure resource lifecycle | Key Vault, DevCenter managed identity | `infra/settings/security/security.yaml:22`, `infra/settings/workload/devcenter.yaml:42-47` |
| Secret Lifecycle Protection           | `enableSoftDelete: true`, `softDeleteRetentionInDays: 7`, `enablePurgeProtection: true` on Key Vault prevent accidental or malicious permanent deletion of secrets; once purge protection is enabled it cannot be disabled | Data Protection     | Confidential   | Azure Key Vault security properties    | DevExP Team | Key Vault RBAC + subscription  | 7-day soft delete window | Key Vault                             | `infra/settings/security/security.yaml:21-25`                                              |
| Audit Logging via Diagnostic Settings | `allLogs` + `AllMetrics` diagnostic settings on Key Vault forward all access, modification, and deletion events to Log Analytics Workspace via `AzureDiagnostics` destination                                              | Audit Control       | Confidential   | Azure Diagnostics JSON / Log Analytics | DevExP Team | Log Analytics workspace access | Log Analytics retention  | Key Vault, Log Analytics              | `src/security/secret.bicep:29-46`                                                          |
...
| Integration Point                 | Source                                                | Target                                     | Data Transferred             | Protocol                         | Security                     |
| --------------------------------- | ----------------------------------------------------- | ------------------------------------------ | ---------------------------- | -------------------------------- | ---------------------------- |
| YAML Config → Bicep               | `infra/settings/**/*.yaml`                            | Bicep modules (3 files)                    | Typed configuration objects  | `loadYamlContent()` compile-time | Repository access            |
| secretValue → Key Vault           | `azd` environment (`KEY_VAULT_SECRET`)                | `Microsoft.KeyVault/vaults/secrets`        | GitHub PAT string            | ARM Deployment API               | `@secure()` + Key Vault RBAC |
| secretIdentifier → DevCenter      | `secret.outputs.AZURE_KEY_VAULT_SECRET_IDENTIFIER`    | DevCenter catalog `secretIdentifier`       | Key Vault secret URI         | Bicep output parameter chain     | `@secure()` parameter        |
| Key Vault logs → Log Analytics    | `Microsoft.KeyVault/vaults` (allLogs)                 | `Microsoft.OperationalInsights/workspaces` | JSON diagnostic records      | Azure Diagnostic Settings        | Log Analytics workspace key  |
| DevCenter metrics → Log Analytics | Azure DevCenter (Azure Monitor agent)                 | Log Analytics Workspace                    | VM telemetry JSON            | Azure Monitor Agent              | Managed identity + workspace |
| Log Analytics ID → Modules        | `monitoring.outputs.AZURE_LOG_ANALYTICS_WORKSPACE_ID` | `security.bicep`, `workload.bicep`         | Workspace resource ID string | Bicep output parameter           | ARM deployment credentials   |
