...
data_layer_reasoning:
  ...
  session_id: 'da1a-2026-03-19-devexp-devbox'
  inputs_validated:
    ...
    dependencies_loaded:
      ...
      - 'fluent.prompt.md'
      - 'bdat-mermaid.prompt.md'
      - 'main.prompt.md (data layer)'
    scan_results_available: true
  evidence_gathered:
    files_scanned: 28
    data_components_found: 42
    component_types_detected: 11
    confidence_threshold: 0.7
    average_confidence: 0.95
  strategy:
    primary_approach:
      'ARM resource type analysis + YAML schema detection in infra/, src/,
      settings/'
    fallback_if_failed:
      'Search for data entities in /settings, /security, /management, /workload
      folders'
    expected_output:
  ...
  gate_checks:
    - criterion: 'Data classification assigned'
      threshold: 'All components have assigned classification'
      status: 'PASS - all components assigned Internal or Confidential'
    - criterion: 'Section 5 mandatory table schema'
      threshold: '10-column schema per 5.N subsection'
      status: 'PASS - all tables use 10-column schema'
    - criterion: 'Source file format'
      threshold: 'path/file.ext:line-range plain text format'
      status: 'PASS - all source references use plain text path'
    - criterion: 'ERD presence'
      threshold: 'erDiagram block immediately after 5.1 table'
      status: 'PASS - ERD generated from Data Entities in 5.1'
    - criterion: 'Section 5 Summary'
      threshold: '### Summary heading after subsection 5.11'
      status: 'PASS - Summary included after 5.11'
    - criterion: 'No fabricated components'
      threshold: 'All components have file evidence'
      status: 'PASS - component-to-file traceability verified'
    - criterion: 'Negative constraint N-1'
      threshold: 'No actual data values or PII in output'
      status: 'PASS - only schema metadata extracted'
    - criterion: 'Negative constraint N-2'
      threshold: 'No fabricated data components'
      status: 'PASS - all components trace to source files'
    - criterion: 'Negative constraint N-7'
      threshold:
        'No Application/Business/Technology components misclassified as Data'
      status: 'PASS - layer boundary rules applied'
  risk_factors:
    - risk: 'Exposing PII/PHI/credentials in output'
      likelihood: 'low'
      mitigation:
        'Only schema metadata included, no actual secret values extracted'
    - risk: 'Missing Section 5 Summary'
      likelihood: 'eliminated'
      mitigation: 'E-016 gate enforced — Summary appended after 5.11'
  step6_proceed_to_documentation: true
  checkpoint:
    file_scan_complete: true
    confidence_scores_calculated: true
    all_11_types_categorized: true
    data_relationships_mapped: true
...
retention policy, freshness SLA, source and consumer systems, and exact source
file reference. The catalog is the authoritative traceability register linking
every data asset to its originating file and line range.
...
All `Source File` values use plain text `path/file.ext:line-range` format. No
markdown links appear in Source File cells.

### 5.1 Data Entities

| Component                       | Description                                                                                                      | Classification | Storage        | Owner  | Retention         | Freshness SLA | Source Systems                     | Consumers                      | Source File                                                   |
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ------ | ----------------- | ------------- | ---------------------------------- | ------------------------------ | ------------------------------------------------------------- |
| ResourceGroup (workload)        | Azure container for all workload resources (`devexp-workload-{env}-{location}-RG`)                               | Internal       | Object Storage | DevExP | indefinite        | batch         | AZD deploy                         | ARM API                        | infra/main.bicep:60-65                                        |
| ResourceGroup (security)        | Logical security landing zone RG (create: false, shares workload RG in dev)                                      | Internal       | Object Storage | DevExP | indefinite        | batch         | AZD deploy                         | ARM API                        | infra/settings/resourceOrganization/azureResources.yaml:17-22 |
| ResourceGroup (monitoring)      | Logical monitoring landing zone RG (create: false, shares workload RG in dev)                                    | Internal       | Object Storage | DevExP | indefinite        | batch         | AZD deploy                         | ARM API                        | infra/settings/resourceOrganization/azureResources.yaml:23-28 |
| LogAnalyticsWorkspace           | Centralized telemetry store (`{prefix}-{uniqueSuffix}`, PerGB2018 SKU)                                           | Internal       | Data Lake      | DevExP | workspace default | real-time     | KV, DevCenter, VNet diagnostics    | Platform teams                 | src/management/logAnalytics.bicep:42-55                       |
| KeyVault                        | Encrypted secrets management store (`contoso-{unique}-kv`, Standard SKU)                                         | Confidential   | Key-Value      | DevExP | 7d soft-delete    | batch         | ARM deploy                         | Catalogs, Workspace            | src/security/keyVault.bicep:46-67                             |
| Secret (gha-token)              | GitHub Actions PAT for private catalog authentication (`text/plain`)                                             | Confidential   | Key-Value      | DevExP | 7d soft-delete    | batch         | KEY_VAULT_SECRET env var           | DC Catalog, Project Catalog    | src/security/secret.bicep:22-33                               |
| DevCenter                       | Azure DevCenter (`devexp-devcenter`, SystemAssigned identity, catalog sync enabled)                              | Internal       | Object Storage | DevExP | indefinite        | batch         | devcenter.yaml, Log Analytics      | Projects, Catalogs             | src/workload/core/devCenter.bicep:182-200                     |
| DevCenterCatalog                | Public Git task catalog (`customTasks`, microsoft/devcenter-catalog, scheduled sync)                             | Internal       | Object Storage | DevExP | indefinite        | batch         | GitHub (public)                    | DevCenter                      | src/workload/core/catalog.bicep:47-65                         |
| EnvironmentType (DC level)      | DevCenter-level environment tiers: dev, staging, UAT                                                             | Internal       | Object Storage | DevExP | indefinite        | batch         | devcenter.yaml                     | Projects                       | src/workload/core/environmentType.bicep:22-35                 |
| Project (eShop)                 | DevCenter project for eShop workload (`catalogItemSyncTypes: EnvironmentDefinition,ImageDefinition`)             | Internal       | Object Storage | DevExP | indefinite        | batch         | DevCenter                          | Teams, Pools                   | src/workload/project/project.bicep:175-200                    |
| ProjectCatalog (environments)   | Private eShop environment definition catalog (Evilazaro/eShop, `/.devcenter/environments`)                       | Confidential   | Object Storage | DevExP | indefinite        | batch         | GitHub (private, KV PAT)           | eShop Project                  | src/workload/project/projectCatalog.bicep:36-55               |
| ProjectCatalog (devboxImages)   | Private eShop image definition catalog (Evilazaro/eShop, `/.devcenter/imageDefinitions`)                         | Confidential   | Object Storage | DevExP | indefinite        | batch         | GitHub (private, KV PAT)           | eShop Project Pools            | src/workload/project/projectCatalog.bicep:36-55               |
| EnvironmentType (project level) | Project-level environment tiers: dev, staging, UAT with SystemAssigned identity                                  | Internal       | Object Storage | DevExP | indefinite        | batch         | DevCenter env types                | Project environments           | src/workload/project/projectEnvironmentType.bicep:30-45       |
| DevBoxPool (backend-engineer)   | Backend developer VM pool (`general_i_32c128gb512ssd_v2`, definition: `eshop-backend-dev`) — currently disabled  | Internal       | Object Storage | DevExP | indefinite        | batch         | Image definition catalog           | eShop backend team             | src/workload/project/project.bicep:301-320                    |
| DevBoxPool (frontend-engineer)  | Frontend developer VM pool (`general_i_16c64gb256ssd_v2`, definition: `eshop-frontend-dev`) — currently disabled | Internal       | Object Storage | DevExP | indefinite        | batch         | Image definition catalog           | eShop frontend team            | src/workload/project/project.bicep:301-320                    |
| VirtualNetwork (eShop)          | Developer VNet (`10.0.0.0/16`, subnet `eShop-subnet` 10.0.1.0/24, Unmanaged type)                                | Internal       | Object Storage | DevExP | indefinite        | batch         | devcenter.yaml connectivity config | NetworkConnection, DevBoxPools | src/connectivity/vnet.bicep:39-93                             |
| NetworkConnection               | Azure AD-joined network connection linking eShop VNet to DevCenter                                               | Internal       | Object Storage | DevExP | indefinite        | batch         | VirtualNetwork                     | DevCenter                      | src/connectivity/networkConnection.bicep:27-50                |
| RoleAssignment (6 types)        | RBAC bindings at Subscription, ResourceGroup, and Project scope (6 assignment bicep modules)                     | Internal       | Object Storage | DevExP | indefinite        | batch         | devcenter.yaml RBAC config         | KV, DevCenter, Projects        | src/identity/devCenterRoleAssignment.bicep:1-55               |
...
| Component                    | Description                                                                                       | Classification | Storage        | Owner  | Retention  | Freshness SLA | Source Systems          | Consumers                        | Source File                                                          |
| ---------------------------- | ------------------------------------------------------------------------------------------------- | -------------- | -------------- | ------ | ---------- | ------------- | ----------------------- | -------------------------------- | -------------------------------------------------------------------- |
| azureResources.yaml          | Declarative RG organization model: 3 landing zone entries, 7-key tag schema                       | Internal       | Object Storage | DevExP | indefinite | batch         | Repo maintainers        | main.bicep (loadYamlContent)     | infra/settings/resourceOrganization/azureResources.yaml:1-67         |
| azureResources.schema.json   | JSON Schema (draft/2020-12) with `resourceGroup` `$defs`: create, name, description, tags         | Internal       | Object Storage | DevExP | indefinite | batch         | Schema authors          | azureResources.yaml validation   | infra/settings/resourceOrganization/azureResources.schema.json:1-166 |
| security.yaml                | Key Vault config model: name, secretName, flags, softDeleteRetentionInDays, tags                  | Confidential   | Object Storage | DevExP | indefinite | batch         | Repo maintainers        | security.bicep (loadYamlContent) | infra/settings/security/security.yaml:1-44                           |
| security.schema.json         | JSON Schema (draft/2020-12) for Key Vault config with name/secretName pattern constraints         | Internal       | Object Storage | DevExP | indefinite | batch         | Schema authors          | security.yaml validation         | infra/settings/security/security.schema.json:1-180                   |
| devcenter.yaml               | Primary workload config model: DevCenter, roles, catalogs, env types, projects, pools (240 lines) | Internal       | Object Storage | DevExP | indefinite | batch         | Repo maintainers        | workload.bicep (loadYamlContent) | infra/settings/workload/devcenter.yaml:1-240                         |
| devcenter.schema.json        | Comprehensive JSON Schema (draft/2020-12) with `$defs` for 14 sub-types (550+ lines)              | Internal       | Object Storage | DevExP | indefinite | batch         | Schema authors          | devcenter.yaml validation        | infra/settings/workload/devcenter.schema.json:1-550                  |
| DevCenterConfig (Bicep UDT)  | User-defined Bicep type for DevCenter resource configuration with identity and role arrays        | Internal       | Object Storage | DevExP | indefinite | batch         | devCenter.bicep authors | devCenter.bicep param binding    | src/workload/core/devCenter.bicep:33-90                              |
| ProjectConfig (Bicep UDT)    | User-defined Bicep type for Project with nested network, identity, catalogs, pools                | Internal       | Object Storage | DevExP | indefinite | batch         | project.bicep authors   | project.bicep param binding      | src/workload/project/project.bicep:41-150                            |
| NetworkSettings (Bicep UDT)  | User-defined type for VNet: name, type, create, addressPrefixes, subnets                          | Internal       | Object Storage | DevExP | indefinite | batch         | vnet.bicep authors      | vnet.bicep param binding         | src/connectivity/vnet.bicep:15-36                                    |
| KeyVaultSettings (Bicep UDT) | User-defined type for Key Vault: name, flags, retentionDays                                       | Confidential   | Object Storage | DevExP | indefinite | batch         | keyVault.bicep authors  | keyVault.bicep param binding     | src/security/keyVault.bicep:15-35                                    |
| RoleAssignment (Bicep UDT)   | User-defined type: azureADGroupId, azureADGroupName, azureRBACRoles[] with scope discriminator    | Internal       | Object Storage | DevExP | indefinite | batch         | identity module authors | identity/\*.bicep modules        | src/workload/core/devCenter.bicep:65-80                              |

### 5.3 Data Stores

| Component                  | Description                                                                                        | Classification | Storage        | Owner     | Retention      | Freshness SLA | Source Systems                     | Consumers                        | Source File                                     |
| -------------------------- | -------------------------------------------------------------------------------------------------- | -------------- | -------------- | --------- | -------------- | ------------- | ---------------------------------- | -------------------------------- | ----------------------------------------------- |
| Azure Key Vault            | Encrypted secrets store (`contoso-{unique}-kv`, Standard SKU, RBAC, purge-protected)               | Confidential   | Key-Value      | DevExP    | 7d soft-delete | batch         | ARM deployment                     | Catalog modules, Project modules | src/security/keyVault.bicep:46-87               |
| Log Analytics Workspace    | Diagnostic log repository (`{prefix}-{uniqueSuffix}`, PerGB2018, AzureActivity solution installed) | Internal       | Data Lake      | DevExP    | Not detected   | real-time     | KV diag, DevCenter diag, VNet diag | Platform engineers, Monitoring   | src/management/logAnalytics.bicep:42-110        |
| Azure Resource Manager     | Subscription-scoped infrastructure state for all ARM resources                                     | Internal       | Object Storage | DevExP    | indefinite     | batch         | AZD deploy                         | ARM consumers, azd env           | infra/main.bicep:1-163                          |
| GitHub (devcenter-catalog) | Public Git repo (microsoft/devcenter-catalog) hosting task definitions at `./Tasks`                | Public         | Object Storage | Microsoft | indefinite     | batch         | GitHub Actions                     | DevCenter DC-level catalog       | src/workload/core/catalog.bicep:47-65           |
| GitHub (eShop)             | Private Git repo (Evilazaro/eShop) hosting environment and image definition YAMLs                  | Confidential   | Object Storage | DevExP    | indefinite     | batch         | eShop team                         | Project catalogs (2 paths)       | src/workload/project/projectCatalog.bicep:36-55 |

### 5.4 Data Flows

| Component                    | Description                                                                                                    | Classification | Storage        | Owner  | Retention         | Freshness SLA | Source Systems      | Consumers                                  | Source File                       |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ------ | ----------------- | ------------- | ------------------- | ------------------------------------------ | --------------------------------- |
| Secret Provisioning Flow     | `${KEY_VAULT_SECRET}` → `main.parameters.json` → `@secure() param secretValue` → KV vault → `gha-token` secret | Confidential   | Key-Value      | DevExP | 7d                | batch         | AZD environment     | Key Vault                                  | infra/main.parameters.json:1-14   |
| Secret Consumption Flow      | `AZURE_KEY_VAULT_SECRET_IDENTIFIER` output → `secretIdentifier` param → catalog `secretIdentifier` field       | Confidential   | Key-Value      | DevExP | session           | batch         | Key Vault           | DC Catalog, Project Catalogs               | src/security/security.bicep:44-52 |
| Monitoring Data Pipeline     | KV allLogs + AllMetrics → diagnostic settings → Log Analytics Workspace                                        | Internal       | Data Lake      | DevExP | workspace default | real-time     | KV, DevCenter, VNet | Log Analytics                              | src/security/secret.bicep:34-53   |
| Configuration Hydration Flow | `azureResources.yaml` + `security.yaml` + `devcenter.yaml` → `loadYamlContent()` → Bicep variables             | Internal       | Object Storage | DevExP | compile-time      | batch         | YAML files          | main.bicep, security.bicep, workload.bicep | infra/main.bicep:30-31            |
| Output Propagation Flow      | Module outputs → parent Bicep outputs → `azure.yaml` hooks → AZD `.env` environment bindings                   | Internal       | Object Storage | DevExP | session           | batch         | All Bicep modules   | AZD CLI, downstream services               | azure.yaml:1-\*                   |

### 5.5 Data Services

| Component                      | Description                                                                                  | Classification | Storage        | Owner  | Retention | Freshness SLA | Source Systems          | Consumers                     | Source File                             |
| ------------------------------ | -------------------------------------------------------------------------------------------- | -------------- | -------------- | ------ | --------- | ------------- | ----------------------- | ----------------------------- | --------------------------------------- |
| Key Vault REST API             | RBAC-authorized secrets CRUD (`4633458b` = KV Secrets User, `b86a8fe4` = KV Secrets Officer) | Confidential   | Key-Value      | DevExP | N/A       | real-time     | ARM identity tokens     | Catalog modules, developers   | src/identity/keyVaultAccess.bicep:1-28  |
| Log Analytics Query API        | Workspace query for diagnostic logs and metrics via Kusto Query Language                     | Internal       | Data Lake      | DevExP | N/A       | real-time     | Log Analytics Workspace | DevOps, monitoring dashboards | src/management/logAnalytics.bicep:42-55 |
| ARM Deployment API             | Resource provisioning and state query at subscription scope (targetScope = 'subscription')   | Internal       | Object Storage | DevExP | N/A       | batch         | AZD CLI                 | All ARM resources             | infra/main.bicep:1                      |
| GitHub API                     | PAT-authenticated read access to private eShop catalogs; unauthenticated for public catalog  | Confidential   | Object Storage | DevExP | N/A       | batch         | gha-token secret        | DC Catalog, Project Catalogs  | src/workload/core/catalog.bicep:47-65   |
| Azure DevCenter API 2025-02-01 | DevCenter management plane: create/manage DevCenters, Projects, Pools, Catalogs              | Internal       | Object Storage | DevExP | N/A       | batch         | ARM deployment          | DevCenter resources           | src/workload/core/devCenter.bicep:182   |

### 5.6 Data Governance

| Component                 | Description                                                                                              | Classification | Storage        | Owner  | Retention         | Freshness SLA | Source Systems                 | Consumers                      | Source File                                                  |
| ------------------------- | -------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ------ | ----------------- | ------------- | ------------------------------ | ------------------------------ | ------------------------------------------------------------ |
| RBAC Authorization Policy | Key Vault RBAC-only access (`enableRbacAuthorization: true`); all six role types use deterministic GUIDs | Internal       | Object Storage | DevExP | indefinite        | batch         | devcenter.yaml roleAssignments | Key Vault, DevCenter, Projects | src/security/keyVault.bicep:58-67                            |
| Universal Tag Schema      | 7-key mandatory tags on all RGs and ARM resources; environment enum-constrained                          | Internal       | Object Storage | DevExP | indefinite        | batch         | YAML configs                   | All ARM resources              | infra/settings/resourceOrganization/azureResources.yaml:5-14 |
| JSON Schema Validation    | Three draft/2020-12 schema files validating all YAML configurations at design-time                       | Internal       | Object Storage | DevExP | indefinite        | batch         | Schema authors                 | Repo CI / design-time tooling  | infra/settings/\*_/_.schema.json:\*                          |
| Landing Zone Organization | Workload / Security / Monitoring logical zones with boolean `create` gate                                | Internal       | Object Storage | DevExP | indefinite        | batch         | azureResources.yaml            | main.bicep conditional logic   | infra/settings/resourceOrganization/azureResources.yaml:1-67 |
| Secure Parameter Handling | `@secure()` on `secretValue` and `secretIdentifier`; suppresses values from ARM activity logs            | Confidential   | Object Storage | DevExP | N/A               | N/A           | Bicep parameter decorators     | ARM deployment engine          | infra/main.bicep:6                                           |
| Diagnostic Audit Policy   | `allLogs + AllMetrics` diagnostic settings on KV, DevCenter, and VNet resources                          | Internal       | Data Lake      | DevExP | workspace default | real-time     | ARM diagnostic settings        | Log Analytics                  | src/security/secret.bicep:34-53                              |

### 5.7 Data Quality Rules

| Component                 | Description                                                                               | Classification | Storage        | Owner  | Retention    | Freshness SLA | Source Systems                                   | Consumers                   | Source File                                                           |
| ------------------------- | ----------------------------------------------------------------------------------------- | -------------- | -------------- | ------ | ------------ | ------------- | ------------------------------------------------ | --------------------------- | --------------------------------------------------------------------- |
| EnvironmentName Length    | `@minLength(2) @maxLength(10)` on `param environmentName string`                          | Internal       | Object Storage | DevExP | compile-time | N/A           | main.bicep                                       | ARM parameter validation    | infra/main.bicep:7                                                    |
| Location Allowlist        | `@allowed` with 17 Azure region strings on `param location string`                        | Internal       | Object Storage | DevExP | compile-time | N/A           | main.bicep                                       | ARM parameter validation    | infra/main.bicep:8-27                                                 |
| Key Vault Name Pattern    | JSON Schema `pattern: ^[a-zA-Z0-9-]{3,24}$` on `keyVault.name`                            | Internal       | Object Storage | DevExP | design-time  | N/A           | security.schema.json                             | YAML authoring tools        | infra/settings/security/security.schema.json:50-60                    |
| Secret Name Pattern       | JSON Schema `pattern: ^[a-zA-Z0-9-]{1,127}$` on `keyVault.secretName`                     | Internal       | Object Storage | DevExP | design-time  | N/A           | security.schema.json                             | YAML authoring tools        | infra/settings/security/security.schema.json:70-80                    |
| SoftDeleteRetention Range | JSON Schema `minimum: 7, maximum: 90` on `softDeleteRetentionInDays`                      | Internal       | Object Storage | DevExP | design-time  | N/A           | security.schema.json                             | YAML authoring tools        | infra/settings/security/security.schema.json:85-95                    |
| GUID Format Validation    | `$defs/guid` pattern `^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-...` on all role and group ID fields | Internal       | Object Storage | DevExP | design-time  | N/A           | devcenter.schema.json                            | YAML authoring tools        | infra/settings/workload/devcenter.schema.json:10-25                   |
| CIDR Block Pattern        | `$defs/cidrBlock` pattern `^(?:\d{1,3}\.){3}\d{1,3}\/\d{1,2}$` on VNet addressPrefixes    | Internal       | Object Storage | DevExP | design-time  | N/A           | devcenter.schema.json                            | YAML authoring tools        | infra/settings/workload/devcenter.schema.json:60-75                   |
| RG Name Pattern           | `pattern: ^[a-zA-Z0-9._-]+$`, `maxLength: 90, minLength: 1` on resourceGroup.name         | Internal       | Object Storage | DevExP | design-time  | N/A           | azureResources.schema.json                       | YAML authoring tools        | infra/settings/resourceOrganization/azureResources.schema.json:55-70  |
| Environment Tag Enum      | `enum: ["dev","test","staging","prod"]` on all resources' `environment` tag               | Internal       | Object Storage | DevExP | design-time  | N/A           | azureResources.schema.json, security.schema.json | All resource tag validators | infra/settings/resourceOrganization/azureResources.schema.json:90-100 |

### 5.8 Master Data

| Component                | Description                                                                                                                                                                                                | Classification | Storage        | Owner     | Retention  | Freshness SLA | Source Systems               | Consumers                                          | Source File                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | --------- | ---------- | ------------- | ---------------------------- | -------------------------------------------------- | ------------------------------------------------------------ |
| Universal Tag Schema     | Canonical 7-key tag definitions applied to all ARM resources (environment, division, team, project, costCenter, owner, landingZone)                                                                        | Internal       | Object Storage | DevExP    | indefinite | batch         | YAML configs                 | azureResources.yaml, security.yaml, devcenter.yaml | infra/settings/resourceOrganization/azureResources.yaml:5-14 |
| Built-in RBAC Role GUIDs | Fixed GUIDs: Contributor (b24988ac), UAA (18d7d88d), KV Secrets User (4633458b), KV Secrets Officer (b86a8fe4), DevBox User (45d50f46), Deployment Env User (18e40d4e), DevCenter Project Admin (331c37c6) | Internal       | Object Storage | Microsoft | indefinite | N/A           | Azure built-in RBAC          | devcenter.yaml, identity modules                   | infra/settings/workload/devcenter.yaml:32-44                 |
| Azure AD Group IDs       | Platform Engineering Team (`5a1d1455-e771-4c19-aa03-fb4a08418f22`), eShop Engineers (`9d42a792-2d74-441d-8bcb-71009371725f`)                                                                               | Confidential   | Object Storage | DevExP    | indefinite | batch         | Entra ID                     | devcenter.yaml, identity modules                   | infra/settings/workload/devcenter.yaml:48-56                 |
| Environment Taxonomy     | Canonical tiers: dev, staging, UAT (project scope); dev, test, staging, prod (schema enum)                                                                                                                 | Internal       | Object Storage | DevExP    | indefinite | N/A           | devcenter.yaml, schema files | All environment type resources                     | infra/settings/workload/devcenter.yaml:72-77                 |
| Azure Region Allowlist   | 17 authorized deployment regions for platform resources                                                                                                                                                    | Internal       | Object Storage | DevExP    | indefinite | N/A           | main.bicep @allowed values   | ARM parameter validation                           | infra/main.bicep:8-27                                        |

### 5.9 Data Transformations

| Component                        | Description                                                                                                              | Classification | Storage        | Owner  | Retention    | Freshness SLA | Source Systems                          | Consumers                                  | Source File                                  |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | -------------- | -------------- | ------ | ------------ | ------------- | --------------------------------------- | ------------------------------------------ | -------------------------------------------- |
| `loadYamlContent()` Hydration    | Compile-time: 3 YAML files → Bicep object variables (`landingZones`, `securitySettings`, `devCenterSettings`)            | Internal       | Object Storage | DevExP | compile-time | batch         | YAML config files                       | main.bicep, security.bicep, workload.bicep | infra/main.bicep:30-31                       |
| `uniqueString()` ID Generation   | Deterministic unique suffix: `uniqueString(rg.id, location, subscription.id, deployer.tenantId)` → unique KV name suffix | Internal       | Object Storage | DevExP | compile-time | batch         | ARM metadata                            | keyVault.bicep name construction           | src/security/keyVault.bicep:8                |
| `resourceNameSuffix` Computation | `${environmentName}-${location}-RG` string concatenation for all three RG names                                          | Internal       | Object Storage | DevExP | compile-time | batch         | ARM parameters                          | main.bicep conditional RG names            | infra/main.bicep:34-55                       |
| `createResourceGroupName` Logic  | Conditional RG name selection: if `landingZones.security.create` then new RG else workload RG                            | Internal       | Object Storage | DevExP | compile-time | batch         | azureResources.yaml create flags        | main.bicep module invocations              | infra/main.bicep:34-55                       |
| Log Analytics Name Truncation    | `maxNameLength = 63 - length(uniqueSuffix) - 1`; `truncatedName = take(name, maxNameLength)`                             | Internal       | Object Storage | DevExP | compile-time | batch         | Log Analytics workspace name param      | logAnalytics.bicep name property           | src/management/logAnalytics.bicep:34-37      |
| Dev Box Definition Name Assembly | Composite key: `~Catalog~${catalog.name}~${imageDefinitionName}` for pool-to-image binding                               | Internal       | Object Storage | DevExP | compile-time | batch         | devcenter.yaml pool imageDefinitionName | projectPool.bicep devBoxDefinitionName     | src/workload/project/projectPool.bicep:73-90 |

### 5.10 Data Contracts

| Component                            | Description                                                                                                | Classification | Storage        | Owner  | Retention  | Freshness SLA | Source Systems                 | Consumers                        | Source File                                                          |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ------ | ---------- | ------------- | ------------------------------ | -------------------------------- | -------------------------------------------------------------------- |
| azureResources.schema.json           | JSON Schema contract for resource organization YAML: requires workload/security/monitoring keys; 166 lines | Internal       | Object Storage | DevExP | indefinite | batch         | Schema authors                 | YAML validation tooling          | infra/settings/resourceOrganization/azureResources.schema.json:1-166 |
| security.schema.json                 | JSON Schema contract for Key Vault config YAML: name pattern, retention range, enum flags; 180 lines       | Internal       | Object Storage | DevExP | indefinite | batch         | Schema authors                 | YAML validation tooling          | infra/settings/security/security.schema.json:1-180                   |
| devcenter.schema.json                | Comprehensive JSON Schema with 14 `$defs` for DevCenter workload config; 550+ lines                        | Internal       | Object Storage | DevExP | indefinite | batch         | Schema authors                 | YAML validation tooling          | infra/settings/workload/devcenter.schema.json:1-550                  |
| ARM Output Contract (main.bicep)     | 10-output consumer contract: 3 RG names, 2 LA workspace IDs, 3 KV outputs, DC name, project array          | Internal       | Object Storage | DevExP | deployment | batch         | All child Bicep modules        | AZD service bindings, azure.yaml | infra/main.bicep:65-160                                              |
| ARM Output Contract (security.bicep) | 3-output contract: `AZURE_KEY_VAULT_NAME`, `AZURE_KEY_VAULT_SECRET_IDENTIFIER`, `AZURE_KEY_VAULT_ENDPOINT` | Confidential   | Object Storage | DevExP | deployment | batch         | keyVault.bicep, secret.bicep   | main.bicep, AZD                  | src/security/security.bicep:44-55                                    |
| ARM Output Contract (workload.bicep) | 2-output contract: `AZURE_DEV_CENTER_NAME` (string), `AZURE_DEV_CENTER_PROJECTS` (array)                   | Internal       | Object Storage | DevExP | deployment | batch         | devCenter.bicep, project.bicep | main.bicep, AZD                  | src/workload/workload.bicep:85-100                                   |
| azure.yaml AZD Service Bindings      | AZD-to-Bicep output map; hooks configuration for pre/post deploy scripts                                   | Internal       | Object Storage | DevExP | indefinite | batch         | main.bicep outputs             | AZD CLI, developer environment   | azure.yaml:1-\*                                                      |

### 5.11 Data Security

| Component                        | Description                                                                                       | Classification | Storage        | Owner  | Retention         | Freshness SLA | Source Systems         | Consumers                    | Source File                           |
| -------------------------------- | ------------------------------------------------------------------------------------------------- | -------------- | -------------- | ------ | ----------------- | ------------- | ---------------------- | ---------------------------- | ------------------------------------- |
| Key Vault RBAC Authorization     | `enableRbacAuthorization: true` on vault properties; no legacy access policies provisioned        | Confidential   | Key-Value      | DevExP | indefinite        | N/A           | ARM security config    | ARM RBAC enforcement         | src/security/keyVault.bicep:58-67     |
| `@secure()` Parameter Decoration | `@secure() param secretValue` and `@secure() param secretIdentifier` suppress from ARM log output | Confidential   | Object Storage | DevExP | N/A               | N/A           | Bicep compiler         | ARM deployment engine        | infra/main.bicep:6                    |
| Soft Delete + Purge Protection   | `enableSoftDelete: true`, `enablePurgeProtection: true` on vault; 7-day retention window          | Confidential   | Key-Value      | DevExP | 7d                | N/A           | security.yaml settings | Key Vault service            | src/security/keyVault.bicep:50-60     |
| KV Diagnostic Audit Logging      | `allLogs + AllMetrics` → Log Analytics; captures all secret access, key operations                | Confidential   | Data Lake      | DevExP | workspace default | real-time     | Key Vault diagnostics  | Log Analytics, SOC           | src/security/secret.bicep:34-53       |
| Deployer Bootstrap Access        | `deployer().objectId` gets `secrets/get,list,set,delete` + `keys/*` at vault creation (bootstrap) | Confidential   | Key-Value      | DevExP | deploy-time       | N/A           | ARM deployer context   | Key Vault bootstrap          | src/security/keyVault.bicep:58-67     |
| Secret URI-Only Output Pattern   | `AZURE_KEY_VAULT_SECRET_IDENTIFIER` outputs only the URI; `secretValue` never appears in outputs  | Confidential   | Object Storage | DevExP | N/A               | N/A           | secret.bicep output    | main.bicep, workload.bicep   | src/security/secret.bicep:50-60       |
| Private Catalog PAT via KV URI   | GitHub PAT transmitted only as `secretIdentifier` (KV URI); never as plaintext in any Bicep file  | Confidential   | Object Storage | DevExP | session           | batch         | Key Vault              | DC Catalog, Project Catalogs | src/workload/core/catalog.bicep:47-65 |
...
| Standard                                            | Enforcement Mechanism                      | Source                                            |
| --------------------------------------------------- | ------------------------------------------ | ------------------------------------------------- |
| Resource Naming: `{prefix}-{uniqueString}-{suffix}` | Bicep `uniqueString()` function            | src/security/keyVault.bicep:8                     |
| Tag Schema: 7 mandatory keys                        | YAML config + ARM tagging                  | infra/settings/\*_/_.yaml                         |
| YAML Schema Validation: JSON Schema draft/2020-12   | Schema `$schema` field on all YAML configs | infra/settings/\*_/_.schema.json                  |
| Secure Parameters: `@secure()` decorator            | Bicep compiler enforcement                 | infra/main.bicep:6, infra/main.parameters.json:10 |
| RBAC-Only Secret Access                             | `enableRbacAuthorization: true`            | src/security/keyVault.bicep:58                    |
| Environment Enumeration: dev/test/staging/prod      | JSON Schema `enum` constraint              | infra/settings/\*_/_.schema.json                  |
...
| ----------------------- | -------------------------------- | ------------------------------------------ | ---------------------------------- | ------------------------------------------------------------ |
| Azure Key Vault         | team: DevExP                     | KV Secrets Officer (`b86a8fe4`)            | Platforms / DevExP                 | infra/settings/security/security.yaml:20-30                  |
| Log Analytics Workspace | team: DevExP                     | Contributor (`b24988ac`)                   | Platforms / DevExP                 | infra/settings/resourceOrganization/azureResources.yaml:5-14 |
| DevCenter               | team: DevExP                     | Contributor + UAA (`b24988ac`, `18d7d88d`) | Platforms / DevExP                 | infra/settings/workload/devcenter.yaml:32-44                 |
| eShop Project           | team: DevExP                     | DevCenter Project Admin (`331c37c6`)       | Platform Engineering Team          | infra/settings/workload/devcenter.yaml:48-56                 |
...
| -------------------------------------- | ---------------- | --------------------------------------------- | ------------------------ | ---------------------------------------------- |
| DevCenter Managed Identity             | ServicePrincipal | Contributor, User Access Administrator        | Subscription             | infra/settings/workload/devcenter.yaml:32-44   |
| DevCenter Managed Identity             | ServicePrincipal | KV Secrets User, KV Secrets Officer           | ResourceGroup            | infra/settings/workload/devcenter.yaml:39-44   |
| Platform Engineering Team (`5a1d1455`) | Group            | DevCenter Project Admin                       | ResourceGroup            | infra/settings/workload/devcenter.yaml:48-56   |
| eShop Engineers (`9d42a792`)           | Group            | Contributor, DevBox User, Deployment Env User | Project                  | infra/settings/workload/devcenter.yaml:100-120 |
...
| -------------------------- | ------------------------------------------------ | --------- | ----------------------------------------- |
| Secret Access Audit        | KV `allLogs` → Log Analytics                     | Real-time | src/security/secret.bicep:34-53           |
| DevCenter Operations Audit | DevCenter `allLogs + AllMetrics` → Log Analytics | Real-time | src/workload/core/devCenter.bicep:219-240 |
| Network Flow Audit         | VNet `allLogs + AllMetrics` → Log Analytics      | Real-time | src/connectivity/vnet.bicep:95-130        |
| ARM Activity Log           | AzureActivity solution in Log Analytics          | Real-time | src/management/logAnalytics.bicep:56-75   |
| Infrastructure State       | Azure Resource Manager (subscription-scoped)     | On deploy | infra/main.bicep:1-163                    |
...
| Section 5 subsections 5.1–5.11 with 10-column schema         | ✅ PASS | All 11 subsections with mandatory 10-column table               |
