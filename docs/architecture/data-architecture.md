...
| Principle                   | Description                                                                                       | Observable Evidence                                                           | Source File                                                   |
| --------------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------- |
| Schema-First Design         | All configuration data is authored against formal JSON Schema contracts before consumption        | yaml-language-server directives in all YAML files; three \*.schema.json files | infra/settings/\*_/_.schema.json:1-\*                         |
| Privacy-by-Design           | Secrets are never stored in plaintext configuration files; all sensitive parameters use @secure() | @secure() on secretValue, secretIdentifier; Key Vault for secret storage      | infra/main.bicep:6-8                                          |
| Infrastructure-as-Data      | Platform configuration is structured data with formal types, constraints, and validation rules    | Bicep user-defined types, @minLength, @allowed decorators                     | src/security/keyVault.bicep:12-36                             |
| RBAC-Only Access            | All data store access uses Azure RBAC authorization; legacy access policies are disabled          | enableRbacAuthorization: true in Key Vault configuration                      | infra/settings/security/security.yaml:30                      |
| Immutable-Data Protection   | Deleted data assets cannot be immediately destroyed; recovery windows are enforced                | enableSoftDelete: true (7 days), enablePurgeProtection: true                  | infra/settings/security/security.yaml:27-29                   |
| Telemetry-by-Default        | All Azure data services emit diagnostic logs and metrics for auditability and observability       | diagnosticSettings resources on Key Vault, DevCenter, VNet, Log Analytics     | src/security/secret.bicep:35-64                               |
| Uniform Governance Metadata | All data assets carry a consistent tag schema for ownership, cost attribution, and classification | Tags applied in all YAML configs and Bicep resource declarations              | infra/settings/resourceOrganization/azureResources.yaml:22-28 |
...
| Level | Label        | Description                                                  | Examples in Codebase                                               |
| ----- | ------------ | ------------------------------------------------------------ | ------------------------------------------------------------------ |
...
| Storage Type            | Instance                      | Scope                           | Data Volume                       | Retention                   |
| ----------------------- | ----------------------------- | ------------------------------- | --------------------------------- | --------------------------- |
...
| ----------------------- | ------------ | ----------------------------------------------- | ------------------------------------------------------------- |
| Secrets management      | ✅ Compliant | Azure Key Vault with RBAC                       | infra/settings/security/security.yaml:27-30                   |
| Audit logging           | ✅ Compliant | allLogs diagnostic settings on all resources    | src/security/secret.bicep:35-64                               |
| Data encryption at rest | ✅ Compliant | Azure-managed encryption in Key Vault           | src/security/keyVault.bicep:44-77                             |
| Access control          | ✅ Compliant | Azure RBAC enforced                             | src/security/keyVault.bicep:68-75                             |
| Retention policy        | ⚠️ Partial   | 7-day soft delete only; no formal policy        | infra/settings/security/security.yaml:29                      |
| Data classification     | ⚠️ Partial   | Environment tags present; no sensitivity labels | infra/settings/resourceOrganization/azureResources.yaml:22-28 |
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------- | -------------- | ----------- | ---------- | ------------- | ------------------- | --------------------------------------------------- | ------------------------------------------------------------- |
| DeploymentParameters         | Infrastructure deployment input entity containing environmentName, location, and secretValue                       | Confidential   | Object Storage | DevExP team | indefinite | batch         | Deployment pipeline | main.bicep, security module                         | infra/main.parameters.json:1-13                               |
| WorkloadResourceGroup        | Resource group organization entity defining workload landing zone name, create flag, description, and tags         | Internal       | Object Storage | DevExP team | indefinite | batch         | azureResources.yaml | main.bicep, workload module                         | infra/settings/resourceOrganization/azureResources.yaml:16-28 |
| SecurityResourceGroup        | Resource group organization entity for Key Vault and security resources (create: false — reuses workload RG)       | Internal       | Object Storage | DevExP team | indefinite | batch         | azureResources.yaml | main.bicep, security module                         | infra/settings/resourceOrganization/azureResources.yaml:32-43 |
| MonitoringResourceGroup      | Resource group organization entity for Log Analytics (create: false — reuses workload RG)                          | Internal       | Object Storage | DevExP team | indefinite | batch         | azureResources.yaml | main.bicep, monitoring module                       | infra/settings/resourceOrganization/azureResources.yaml:47-58 |
| KeyVaultConfiguration        | Key Vault settings entity defining name, purge protection, soft delete, RBAC authorization, and secret name        | Confidential   | Object Storage | DevExP team | indefinite | batch         | security.yaml       | security.bicep, keyVault.bicep                      | infra/settings/security/security.yaml:21-38                   |
| DevCenterConfiguration       | Dev Center core settings entity defining name, catalog sync status, network status, and monitor agent status       | Internal       | Object Storage | DevExP team | indefinite | batch         | devcenter.yaml      | workload.bicep, devCenter.bicep                     | infra/settings/workload/devcenter.yaml:20-26                  |
| ProjectConfiguration         | eShop project entity defining team network, identity, RBAC roles, pools, environment types, and catalog references | Internal       | Object Storage | DevExP team | indefinite | batch         | devcenter.yaml      | project.bicep, projectPool.bicep                    | infra/settings/workload/devcenter.yaml:72-230                 |
| PoolConfiguration            | Dev Box pool entity (backend-engineer, frontend-engineer) defining image definition name and VM SKU                | Internal       | Object Storage | DevExP team | indefinite | batch         | devcenter.yaml      | projectPool.bicep                                   | infra/settings/workload/devcenter.yaml:135-142                |
| CatalogConfiguration         | Git repository catalog entity defining catalog name, type (gitHub/adoGit), URI, branch, and path                   | Internal       | Object Storage | DevExP team | indefinite | batch         | devcenter.yaml      | catalog.bicep, projectCatalog.bicep                 | infra/settings/workload/devcenter.yaml:52-58                  |
| EnvironmentTypeConfiguration | Deployment environment entity (dev, staging, UAT) with deployment target subscription binding                      | Internal       | Object Storage | DevExP team | indefinite | batch         | devcenter.yaml      | environmentType.bicep, projectEnvironmentType.bicep | infra/settings/workload/devcenter.yaml:62-68                  |
...
| Component               | Description                                                                                                                                                   | Classification | Storage        | Owner       | Retention  | Freshness SLA | Source Systems                    | Consumers                                  | Source File                                                         |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ----------- | ---------- | ------------- | --------------------------------- | ------------------------------------------ | ------------------------------------------------------------------- |
| AzureResourcesSchema    | JSON Schema (draft 2020-12) defining required fields, types, constraints, and additionalProperties:false for resource group config                            | Internal       | Object Storage | DevExP team | indefinite | batch         | azureResources.schema.json        | azureResources.yaml (yaml-language-server) | infra/settings/resourceOrganization/azureResources.schema.json:1-\* |
| SecuritySchema          | JSON Schema (draft 2020-12) defining Key Vault configuration structure with $defs for tags, security settings, and required fields                            | Confidential   | Object Storage | DevExP team | indefinite | batch         | security.schema.json              | security.yaml (yaml-language-server)       | infra/settings/security/security.schema.json:1-\*                   |
| DevCenterSchema         | JSON Schema (draft 2020-12) defining Dev Center configuration including $defs for guid, enabledStatus, roleAssignment, rbacRole, and nested project structure | Internal       | Object Storage | DevExP team | indefinite | batch         | devcenter.schema.json             | devcenter.yaml (yaml-language-server)      | infra/settings/workload/devcenter.schema.json:1-\*                  |
| KeyVaultBicepTypeModel  | Bicep user-defined types KeyVaultSettings and KeyVaultConfig enforcing strongly-typed parameters for Key Vault deployment                                     | Confidential   | Object Storage | DevExP team | indefinite | batch         | keyVault.bicep type declarations  | keyVault.bicep resource block              | src/security/keyVault.bicep:12-36                                   |
| DevCenterBicepTypeModel | Bicep user-defined types for DevCenterConfig, Catalog, Identity, RoleAssignment, OrgRoleType, EnvironmentTypeConfig                                           | Internal       | Object Storage | DevExP team | indefinite | batch         | devCenter.bicep type declarations | devCenter.bicep resource block             | src/workload/core/devCenter.bicep:35-148                            |

### 5.3 Data Stores

| Component                  | Description                                                                                                                                 | Classification | Storage        | Owner       | Retention      | Freshness SLA | Source Systems                                                   | Consumers                                          | Source File                                  |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ----------- | -------------- | ------------- | ---------------------------------------------------------------- | -------------------------------------------------- | -------------------------------------------- |
| AzureKeyVault              | Azure-managed secret store (SKU: Standard, family A) with RBAC authorization, soft delete (7d), and purge protection enabled                | Confidential   | Key-Value      | DevExP team | 7d soft delete | real-time     | security.bicep provisioning                                      | DevCenter (secretIdentifier), dev tools at runtime | src/security/keyVault.bicep:44-77            |
| LogAnalyticsWorkspace      | Azure Operational Insights workspace (PerGB2018 SKU) collecting allLogs and AllMetrics from Key Vault, DevCenter, and VNet                  | Internal       | Data Lake      | DevExP team | 30d (default)  | real-time     | Key Vault, DevCenter.diagnosticSettings, VNet.diagnosticSettings | Log Analytics API, Azure Monitor alerts            | src/management/logAnalytics.bicep:38-52      |
| YAMLConfigurationFileStore | File-based declarative configuration store: three YAML documents (azureResources.yaml, security.yaml, devcenter.yaml) under infra/settings/ | Internal       | Object Storage | DevExP team | indefinite     | batch         | Manual authoring, Git commits                                    | Bicep loadYamlContent() at compile time            | infra/settings/workload/devcenter.yaml:1-350 |

### 5.4 Data Flows

| Component                   | Description                                                                                                                                                                   | Classification | Storage      | Owner       | Retention    | Freshness SLA | Source Systems                                                                            | Consumers                                    | Source File                     |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------ | ----------- | ------------ | ------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------- | ------------------------------- |
| YAMLConfigLoadingFlow       | Compile-time flow: Bicep loadYamlContent() reads YAML files into typed variables for parameterization of all module deployments                                               | Internal       | Not detected | DevExP team | Not detected | batch         | azureResources.yaml, security.yaml, devcenter.yaml                                        | main.bicep, security.bicep, workload.bicep   | src/workload/workload.bicep:46  |
| SecretProvisioningFlow      | Deployment-time flow: @secure() secretValue traverses main.bicep → security.bicep → secret.bicep → Key Vault secret resource                                                  | Confidential   | Key-Value    | DevExP team | Not detected | batch         | main.parameters.json (KEY_VAULT_SECRET)                                                   | Key Vault secret, DevCenter secretIdentifier | src/security/secret.bicep:1-30  |
| DiagnosticTelemetryFlow     | Runtime flow: Key Vault, DevCenter, VNet, and Log Analytics emit allLogs + AllMetrics to the Log Analytics Workspace via diagnosticSettings                                   | Internal       | Data Lake    | DevExP team | 30d          | real-time     | Microsoft.KeyVault, Microsoft.DevCenter, Microsoft.Network, Microsoft.OperationalInsights | Log Analytics Workspace                      | src/security/secret.bicep:35-64 |
| ModuleOutputPropagationFlow | Deployment-time chained flow: monitoring outputs WORKSPACE_ID → security + workload; security outputs KV_NAME, SECRET_IDENTIFIER → workload; workload outputs DEV_CENTER_NAME | Internal       | Not detected | DevExP team | Not detected | batch         | monitoring.bicep, security.bicep                                                          | workload.bicep, main.bicep outputs           | infra/main.bicep:102-165        |

### 5.5 Data Services

| Component               | Description                                                                                                                          | Classification | Storage   | Owner           | Retention     | Freshness SLA | Source Systems            | Consumers                                 | Source File                             |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | -------------- | --------- | --------------- | ------------- | ------------- | ------------------------- | ----------------------------------------- | --------------------------------------- |
| AzureKeyVaultSecretsAPI | Azure Key Vault REST API (api-version 2025-05-01) for secret lifecycle operations: get, list, set, delete, backup, restore, recover  | Confidential   | Key-Value | Microsoft Azure | indefinite    | real-time     | Azure control plane       | DevCenter provisioning, developer tooling | src/security/keyVault.bicep:44-77       |
| AzureLogAnalyticsAPI    | Azure Log Analytics REST API and Kusto Query Language (KQL) interface for telemetry query, dashboard data, and monitoring operations | Internal       | Data Lake | Microsoft Azure | 30d (default) | real-time     | Azure diagnostic pipeline | Azure Monitor, operators, dashboards      | src/management/logAnalytics.bicep:38-52 |

### 5.6 Data Governance

| Component              | Description                                                                                                                                                         | Classification | Storage        | Owner       | Retention  | Freshness SLA | Source Systems                 | Consumers                                               | Source File                                                   |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ----------- | ---------- | ------------- | ------------------------------ | ------------------------------------------------------- | ------------------------------------------------------------- |
| ResourceTaggingPolicy  | Uniform Azure resource tag schema applied to all resources: environment (dev/test/staging/prod), division, team, project, costCenter, owner, landingZone, resources | Internal       | Object Storage | DevExP team | indefinite | batch         | All YAML config files          | Azure Cost Management, Azure Policy, resource inventory | infra/settings/resourceOrganization/azureResources.yaml:22-28 |
| RBACAuthorizationModel | Azure RBAC role assignment model for Key Vault (Secrets User/Officer) and Dev Center (Contributor, Project Admin, Dev Box User, Deployment Environment User)        | Confidential   | Object Storage | DevExP team | indefinite | batch         | devcenter.yaml, keyVault.bicep | Azure RBAC system, identity.bicep modules               | infra/settings/workload/devcenter.yaml:40-69                  |
| LandingZoneSegregation | Azure Landing Zone data domain segregation: workload/security/monitoring resource groups with explicit create flags controlling topology                            | Internal       | Object Storage | DevExP team | indefinite | batch         | azureResources.yaml            | main.bicep resource group declarations                  | infra/settings/resourceOrganization/azureResources.yaml:15-58 |

### 5.7 Data Quality Rules

| Component                  | Description                                                                                                                                                    | Classification | Storage        | Owner       | Retention  | Freshness SLA | Source Systems                  | Consumers                                     | Source File                                                         |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ----------- | ---------- | ------------- | ------------------------------- | --------------------------------------------- | ------------------------------------------------------------------- |
| JSONSchemaValidation       | JSON Schema (draft 2020-12) validation rules: required fields, additionalProperties:false, type constraints, pattern matching on GUIDs, enum bounds            | Internal       | Object Storage | DevExP team | indefinite | batch         | \*.schema.json files            | YAML authoring tooling (yaml-language-server) | infra/settings/security/security.schema.json:1-\*                   |
| BicepParameterValidation   | Bicep decorator-based validation: @minLength(2) @maxLength(10) on environmentName, @allowed([16 regions]) on location, @minLength(1) on critical string params | Internal       | Object Storage | DevExP team | indefinite | batch         | infra/main.bicep                | Bicep compiler (build-time rejection)         | infra/main.bicep:3-28                                               |
| YAMLSchemaServerValidation | yaml-language-server directive in all YAML files providing real-time IDE validation against companion schema, preventing syntactically invalid configuration   | Internal       | Object Storage | DevExP team | indefinite | batch         | All \*.yaml configuration files | VS Code YAML extension, CI linting            | infra/settings/workload/devcenter.yaml:1                            |
| EnvironmentEnumConstraint  | Enum constraint restricting environment tag to: dev, test, staging, prod across all three JSON schemas and implicitly in YAML configuration                    | Internal       | Object Storage | DevExP team | indefinite | batch         | All \*.schema.json files        | YAML validation, Bicep compilation            | infra/settings/resourceOrganization/azureResources.schema.json:1-\* |

### 5.8 Master Data

| Component                | Description                                                                                                                                                                                                                                                                         | Classification | Storage        | Owner           | Retention  | Freshness SLA | Source Systems                     | Consumers                                       | Source File                                                         |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | --------------- | ---------- | ------------- | ---------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------- |
| ResourceNamingConvention | Canonical naming pattern: `${name}-${uniqueString(resourceGroup().id, location, subscriptionId, tenantId)}-suffix` ensuring globally unique, deterministic resource names                                                                                                           | Internal       | Object Storage | DevExP team     | indefinite | batch         | keyVault.bicep, logAnalytics.bicep | All resource naming in Bicep modules            | src/security/keyVault.bicep:10                                      |
| EnvironmentTaxonomy      | Authoritative environment vocabulary: dev, test, staging, prod — the canonical classification set referenced by all three JSON Schema contracts and all YAML tag sections                                                                                                           | Internal       | Object Storage | DevExP team     | indefinite | batch         | All \*.schema.json files           | All YAML configs, Bicep parameter validation    | infra/settings/resourceOrganization/azureResources.schema.json:1-\* |
| RBACRoleDefinitions      | Canonical Azure RBAC role GUID registry: Contributor (b24988ac), User Access Administrator (18d7d88d), Key Vault Secrets User (4633458b), Key Vault Secrets Officer (b86a8fe4), DevCenter Project Admin (331c37c6), Dev Box User (45d50f46), Deployment Environment User (18e40d4e) | Internal       | Object Storage | Microsoft Azure | indefinite | Not detected  | devcenter.yaml role assignments    | devCenterRoleAssignment.bicep, identity modules | infra/settings/workload/devcenter.yaml:29-69                        |

### 5.9 Data Transformations

| Component                 | Description                                                                                                                                                                              | Classification | Storage      | Owner       | Retention    | Freshness SLA | Source Systems                             | Consumers                                                       | Source File                       |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------ | ----------- | ------------ | ------------- | ------------------------------------------ | --------------------------------------------------------------- | --------------------------------- |
| YAMLToBicepTransformation | Bicep built-in loadYamlContent() function transforming YAML document trees into strongly-typed Bicep objects at compile time — no runtime overhead                                       | Internal       | Not detected | DevExP team | Not detected | batch         | infra/settings/\*.yaml                     | All module parameter bindings in workload.bicep, security.bicep | src/workload/workload.bicep:45-46 |
| UniqueStringHashing       | uniqueString(resourceGroup().id, location, subscription().subscriptionId, deployer().tenantId) — deterministic hash producing a stable unique suffix for global resource name uniqueness | Internal       | Not detected | DevExP team | Not detected | batch         | Azure Resource Manager context             | keyVault.bicep name construction, logAnalytics.bicep name       | src/security/keyVault.bicep:10    |
| ResourceNameConstruction  | Bicep string interpolation template transforming organization config + environment context into composite resource names: `${landingZones.security.name}-${resourceNameSuffix}`          | Internal       | Not detected | DevExP team | Not detected | batch         | azureResources.yaml, deployment parameters | Resource group resource declarations in main.bicep              | infra/main.bicep:30-46            |

### 5.10 Data Contracts

| Component              | Description                                                                                                                                                                                                                            | Classification | Storage        | Owner       | Retention  | Freshness SLA | Source Systems             | Consumers                                 | Source File                                                         |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | -------------- | ----------- | ---------- | ------------- | -------------------------- | ----------------------------------------- | ------------------------------------------------------------------- |
| AzureResourcesContract | Formal JSON Schema (draft 2020-12) contract specifying the allowed data structure for azureResources.yaml: workload, security, monitoring resource group objects with required fields and tag shapes                                   | Internal       | Object Storage | DevExP team | indefinite | batch         | azureResources.schema.json | azureResources.yaml, yaml-language-server | infra/settings/resourceOrganization/azureResources.schema.json:1-\* |
| SecurityContract       | Formal JSON Schema (draft 2020-12) contract specifying security.yaml structure: create flag, keyVault config object with enablePurgeProtection, enableSoftDelete, softDeleteRetentionInDays, enableRbacAuthorization, secretName, tags | Confidential   | Object Storage | DevExP team | indefinite | batch         | security.schema.json       | security.yaml, yaml-language-server       | infra/settings/security/security.schema.json:1-\*                   |
| DevCenterContract      | Formal JSON Schema (draft 2020-12) contract specifying the full devcenter.yaml structure including nested $defs for guid, enabledStatus, roleAssignment, rbacRole, catalog, pool, project, network, subnet                             | Internal       | Object Storage | DevExP team | indefinite | batch         | devcenter.schema.json      | devcenter.yaml, yaml-language-server      | infra/settings/workload/devcenter.schema.json:1-\*                  |

### 5.11 Data Security

| Component                | Description                                                                                                                                                                                                           | Classification | Storage      | Owner           | Retention    | Freshness SLA | Source Systems                                      | Consumers                              | Source File                                 |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------ | --------------- | ------------ | ------------- | --------------------------------------------------- | -------------------------------------- | ------------------------------------------- |
| KeyVaultAtRestEncryption | Azure-managed AES-256 encryption at rest for all Key Vault secrets and keys (SKU: Standard, family A) — no customer-managed key detected                                                                              | Confidential   | Key-Value    | Microsoft Azure | indefinite   | real-time     | Azure Key Vault service                             | Key Vault stored secrets               | src/security/keyVault.bicep:63-68           |
| SoftDeleteProtection     | enableSoftDelete: true with softDeleteRetentionInDays: 7 providing a 7-day recovery window for accidentally deleted Key Vault secrets                                                                                 | Confidential   | Key-Value    | DevExP team     | 7d           | Not detected  | security.yaml, keyVault.bicep                       | Key Vault delete operations            | infra/settings/security/security.yaml:27-29 |
| PurgeProtection          | enablePurgeProtection: true preventing permanent destruction of deleted Keys and Vaults during the retention window                                                                                                   | Confidential   | Key-Value    | DevExP team     | indefinite   | Not detected  | security.yaml, keyVault.bicep                       | Key Vault purge operations             | infra/settings/security/security.yaml:27    |
| RBACKeyVaultAccess       | enableRbacAuthorization: true on Key Vault — disables legacy access policies and mandates Azure RBAC for all secret access; roles: Key Vault Secrets User, Key Vault Secrets Officer                                  | Confidential   | Key-Value    | DevExP team     | indefinite   | Not detected  | security.yaml:30, devcenter.yaml:36-40              | Azure RBAC system                      | infra/settings/security/security.yaml:30    |
| SecureParameterHandling  | @secure() Bicep decorator applied to secretValue and secretIdentifier parameters preventing parameter values from appearing in Azure deployment history, state files, or logs                                         | Confidential   | Not detected | DevExP team     | Not detected | batch         | infra/main.bicep:7, workload.bicep:15               | Bicep compiler — omits from output     | infra/main.bicep:6-8                        |
| AuditLogging             | allLogs + AllMetrics diagnosticSettings resources on Key Vault (via secret.bicep), Dev Center (via devCenter.bicep), VNet (via vnet.bicep), and Log Analytics (self-referencing) streaming to Log Analytics Workspace | Internal       | Data Lake    | DevExP team     | 30d          | real-time     | Key Vault, DevCenter, VNet, Log Analytics resources | Log Analytics Workspace, Azure Monitor | src/security/secret.bicep:35-64             |
...
| ------------------------ | ------------------------------------ | ----------------------------------------- | ----------------------------------------- |
| Key Vault name           | `{name}-{uniqueString}-kv`           | `contoso-abc123de-kv`                     | src/security/keyVault.bicep:50            |
| Log Analytics name       | `{name}-{uniqueString}`              | `logAnalytics-def456gh`                   | src/management/logAnalytics.bicep:33-36   |
| Resource Group name      | `{org}-{env}-{location}-RG`          | `devexp-workload-dev-eastus-RG`           | infra/main.bicep:35-40                    |
| Diagnostic Settings name | `{resourceName}-diagnostic-settings` | `contoso-abc123de-kv-diagnostic-settings` | src/security/secret.bicep:37              |
| Dev Center name          | `devexp-devcenter`                   | `devexp-devcenter`                        | infra/settings/workload/devcenter.yaml:21 |
...
| Pattern              | Flow Type           | Source                                | Target                   | Processing                             | Contract                             | Quality Gate                           |
| -------------------- | ------------------- | ------------------------------------- | ------------------------ | -------------------------------------- | ------------------------------------ | -------------------------------------- |
| YAML Config Loading  | Batch ETL           | YAML config files                     | Bicep variable objects   | loadYamlContent() compile-time parsing | JSON Schema (yaml-language-server)   | JSON Schema validation at authoring    |
| Secret Provisioning  | Request/Response    | Deployment parameter                  | Key Vault secret         | Direct write via ARM API               | @secure() parameter contract         | No detected validation gate post-write |
| Diagnostic Telemetry | Real-time Streaming | Azure resources (KV, DevCenter, VNet) | Log Analytics Workspace  | Azure Monitor diagnostic pipeline      | diagnosticSettings resource contract | No detected alerting or anomaly rules  |
| Module Output Chain  | Request/Response    | Bicep module outputs                  | Parent module parameters | ARM deployment output binding          | Bicep typed output declarations      | Type checking at Bicep compile time    |
...
| Resource                | Access Model                               | Roles Granted                                                                          | Scope                   | Source                                         |
| ----------------------- | ------------------------------------------ | -------------------------------------------------------------------------------------- | ----------------------- | ---------------------------------------------- |
| Azure Key Vault         | Azure RBAC (enableRbacAuthorization: true) | Key Vault Secrets User, Key Vault Secrets Officer                                      | ResourceGroup           | infra/settings/security/security.yaml:30       |
| Dev Center              | Azure RBAC                                 | Contributor, User Access Administrator                                                 | Subscription            | infra/settings/workload/devcenter.yaml:29-35   |
| Dev Center Projects     | Azure RBAC                                 | DevCenter Project Admin                                                                | ResourceGroup           | infra/settings/workload/devcenter.yaml:54-58   |
| eShop Project           | Azure RBAC                                 | Contributor, Dev Box User, Deployment Environment User, Key Vault Secrets User/Officer | Project / ResourceGroup | infra/settings/workload/devcenter.yaml:118-140 |
| Log Analytics Workspace | Azure RBAC (default)                       | Not detected — implicit workspace contributor                                          | ResourceGroup           | src/management/logAnalytics.bicep:38-52        |

### Audit & Compliance

| Audit Mechanism       | Resources Covered                        | Destination                | Retention   | Source                                    |
| --------------------- | ---------------------------------------- | -------------------------- | ----------- | ----------------------------------------- |
| Key Vault allLogs     | Azure Key Vault (all operations)         | Log Analytics Workspace    | 30d default | src/security/secret.bicep:35-64           |
| Key Vault AllMetrics  | Azure Key Vault (performance metrics)    | Log Analytics Workspace    | 30d default | src/security/secret.bicep:35-64           |
| Dev Center allLogs    | Microsoft.DevCenter/devcenters           | Log Analytics Workspace    | 30d default | src/workload/core/devCenter.bicep:175-200 |
| Dev Center AllMetrics | Microsoft.DevCenter/devcenters           | Log Analytics Workspace    | 30d default | src/workload/core/devCenter.bicep:175-200 |
| VNet allLogs          | Microsoft.Network/virtualNetworks        | Log Analytics Workspace    | 30d default | src/connectivity/vnet.bicep:55-75         |
| Log Analytics (self)  | Microsoft.OperationalInsights/workspaces | Self-referencing workspace | 30d default | src/management/logAnalytics.bicep:60-85   |
