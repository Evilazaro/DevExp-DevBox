...
| [7](#7-architecture-standards)    | 📐 Architecture Standards     | 12 enforced standards · 6 governance domains           |
...
This section documents the business architecture standards, naming conventions,
governance rules, and quality requirements enforced across the DevExp-DevBox
platform. Standards ensure consistency, maintainability, and compliance across
all business-layer components and their downstream realizations. While no
dedicated standards documentation directory exists, standards are rigorously
enforced through JSON Schema contracts, Bicep parameter decorators, GitHub Issue
Form templates, and codified contribution guidelines in `CONTRIBUTING.md`.

Business architecture standards in DevExp-DevBox fall into five governance
domains: (1) Work Item Governance — enforcing the Epic → Feature → Task delivery
hierarchy and mandatory label taxonomy; (2) Resource Organization — Landing Zone
segregation, naming conventions, and mandatory tagging; (3) Security Posture —
Key Vault security baseline and least-privilege RBAC assignments; (4) Deployment
Quality — Definition of Done tiers and validation requirements; and (5)
Documentation — docs-as-code co-location and module documentation completeness.

### 📏 7.1 Business Standards Summary

The following standards are enforced through source code, configuration schemas,
and contribution governance documentation:

| 📏 Standard                                         | ⚙️ Enforcement Mechanism                                                   | 📄 Source Reference                                    |
| --------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------ |
| Epic → Feature → Task Hierarchy                     | GitHub Issue Form templates with mandatory fields                          | `.github/ISSUE_TEMPLATE/epic.yml`, `CONTRIBUTING.md`   |
| Mandatory Issue Labeling (type, area, priority, status) | GitHub Issue Form dropdowns                                            | `.github/ISSUE_TEMPLATE/*.yml`                         |
| Landing Zone Segregation (Workload, Security, Monitoring) | JSON Schema + YAML configuration                                     | `infra/settings/resourceOrganization/azureResources.yaml` |
| Resource Naming: `{name}-{env}-{location}-RG`       | Bicep variable declarations                                                | `infra/main.bicep`                                     |
| Mandatory 7-Tag Resource Tagging                     | JSON Schema `required` constraints on all landing zone definitions         | `azureResources.schema.json`                           |
| Key Vault Security Baseline                          | JSON Schema `const` and `default` values                                   | `security.schema.json`, `security.yaml`                |
| RBAC Least-Privilege Scoping                         | Scoped role assignments with GUID references                               | `devcenter.yaml`, `devcenter.schema.json`              |
| Bicep Module Parameterization                        | `@description`, `@allowed`, `@secure`, `@minLength`, `@maxLength` decorators | `infra/main.bicep`                                     |
| Definition of Done (Task/Feature/Epic tiers)         | Contribution guidelines + Issue Form checklists                            | `CONTRIBUTING.md`                                      |
| Documentation-as-Code Co-location                    | PR review gate requirement                                                 | `CONTRIBUTING.md`                                      |
| Branch Naming Convention                             | Prefix-based naming guidelines                                             | `CONTRIBUTING.md`                                      |
| Pull Request Content Requirements                    | Mandatory issue reference, summary, and validation evidence                | `CONTRIBUTING.md`                                      |

### 📋 7.2 Work Item Governance Standards

#### 7.2.1 Delivery Hierarchy Standard

All work items must follow the three-level delivery hierarchy: **Epics** deliver
measurable business outcomes (capabilities), **Features** deliver concrete,
testable deliverables within an Epic, and **Tasks** are small, verifiable units
of work inside a Feature. Every Feature must link to a parent Epic, and every
Task must link to a parent Feature. This hierarchy is enforced through GitHub
Issue Form templates that include mandatory parent-link fields.

| 🏛️ Attribute       | 📝 Value                                                                                   |
| ------------------ | ------------------------------------------------------------------------------------------ |
| **Standard Name**  | Epic → Feature → Task Delivery Hierarchy                                                   |
| **Enforcement**    | GitHub Issue Form templates with required parent-link dropdowns                             |
| **Scope**          | All work items in the repository                                                           |
| **Source**         | `CONTRIBUTING.md` (lines 9–44), `.github/ISSUE_TEMPLATE/epic.yml`, `feature.yml`, `task.yml` |

#### 7.2.2 Issue Labeling Standard

Every issue must carry four mandatory label categories: **Type** (`type:epic`,
`type:feature`, `type:task`), **Area** (`area:dev-box`, `area:dev-center`,
`area:networking`, etc.), **Priority** (`priority:p0`, `priority:p1`,
`priority:p2`), and **Status** (workflow: `status:triage` → `status:ready` →
`status:in-progress` → `status:done`). Labels are populated through GitHub Issue
Form dropdown fields to prevent free-text entry errors.

| 🏛️ Attribute       | 📝 Value                                                            |
| ------------------ | ------------------------------------------------------------------- |
| **Standard Name**  | Mandatory Four-Category Issue Labeling                              |
| **Enforcement**    | GitHub Issue Form dropdowns (type, area, priority, status)          |
| **Scope**          | All issues and work items                                           |
| **Source**         | `CONTRIBUTING.md` (lines 19–44), `.github/ISSUE_TEMPLATE/*.yml`     |

### 🏗️ 7.3 Resource Organization Standards

#### 7.3.1 Landing Zone Segregation Standard

All Azure resources must be organized into three functional landing zones:
**Workload** (DevCenter, Projects, Dev Box Pools), **Security** (Key Vault,
secrets), and **Monitoring** (Log Analytics Workspace, diagnostic settings).
Each landing zone corresponds to a dedicated resource group and can be
independently toggled via `create` flags in the YAML configuration. The JSON
Schema enforces name patterns (`^[a-zA-Z0-9._-]+$`) and length constraints
(1–90 characters) on all resource group names.

| 🏛️ Attribute       | 📝 Value                                                                                     |
| ------------------ | -------------------------------------------------------------------------------------------- |
| **Standard Name**  | Three-Zone Landing Zone Segregation                                                          |
| **Enforcement**    | JSON Schema constraints + Bicep conditional resource declarations                            |
| **Scope**          | All Azure resource group organization                                                        |
| **Source**         | `infra/settings/resourceOrganization/azureResources.yaml`, `azureResources.schema.json`      |

#### 7.3.2 Resource Naming Convention Standard

All resource group names must follow the pattern
`{resourceName}-{environmentName}-{location}-RG`. The environment name is
constrained to 2–10 characters via Bicep `@minLength` and `@maxLength`
decorators. Location is constrained to 16 approved Azure regions via `@allowed`
decorator. This naming convention prevents cross-environment name collisions and
enables consistent resource identification across deployments.

| 🏛️ Attribute       | 📝 Value                                                        |
| ------------------ | --------------------------------------------------------------- |
| **Standard Name**  | Parameterized Resource Naming Convention                        |
| **Enforcement**    | Bicep variable declarations with parameter decorators           |
| **Pattern**        | `{resourceName}-{environmentName}-{location}-RG`               |
| **Source**         | `infra/main.bicep` (lines 38–62)                                |

#### 7.3.3 Mandatory Tagging Standard

All Azure resources must carry seven mandatory tags: **environment** (enum:
`dev`, `test`, `staging`, `prod`), **division**, **team**, **project**,
**costCenter**, **owner**, and **landingZone** (enum: `DevCenter`, `KeyVault`,
`LogAnalytics`, `Project`, `Network`, `ResourceGroup`). The JSON Schema enforces
all seven tags as `required` properties with type and enum constraints,
preventing deployment without complete tagging metadata.

| 🏛️ Attribute       | 📝 Value                                                                        |
| ------------------ | ------------------------------------------------------------------------------- |
| **Standard Name**  | Seven-Tag Mandatory Resource Tagging                                            |
| **Enforcement**    | JSON Schema `required` array + `enum` constraints on tag values                 |
| **Scope**          | All Azure resources across all landing zones                                    |
| **Source**         | `azureResources.schema.json` (lines 58–125), `security.yaml`, `devcenter.yaml` |

### 🔒 7.4 Security Standards

#### 7.4.1 Key Vault Security Baseline Standard

All Azure Key Vault instances must enforce four mandatory security controls:
`enablePurgeProtection: true`, `enableSoftDelete: true`,
`softDeleteRetentionInDays` (7–90 days, default 90),
`enableRbacAuthorization: true`. These settings are enforced through JSON Schema
`const` and `default` values in `security.schema.json`, preventing deployment of
non-compliant Key Vault configurations. Key Vault names must match the pattern
`^[a-zA-Z0-9-]{3,24}$` and secret names must match `^[a-zA-Z0-9-]{1,127}$`.

| 🏛️ Attribute       | 📝 Value                                                                     |
| ------------------ | ---------------------------------------------------------------------------- |
| **Standard Name**  | Mandatory Key Vault Security Baseline                                        |
| **Enforcement**    | JSON Schema constraints (`const`, `default`, `pattern`)                      |
| **Scope**          | All Azure Key Vault deployments                                              |
| **Source**         | `infra/settings/security/security.schema.json`, `security.yaml`              |

#### 7.4.2 Least-Privilege RBAC Standard

All role assignments must specify a GUID-format role identifier, a display name,
and a scoping level (`Subscription`, `ResourceGroup`, `Project`, `Tenant`, or
`ManagementGroup`). Separate role assignment blocks are required for each
organizational role type. Role assignments must follow the principle of least
privilege — each actor group receives only the minimum permissions required for
its operational scope, as enforced through the structured role assignment
schema in `devcenter.yaml`.

| 🏛️ Attribute       | 📝 Value                                                                     |
| ------------------ | ---------------------------------------------------------------------------- |
| **Standard Name**  | Scoped Least-Privilege RBAC Assignment                                       |
| **Enforcement**    | JSON Schema with GUID pattern validation + scope enum constraint             |
| **Scope**          | All Azure RBAC role assignments                                              |
| **Source**         | `infra/settings/workload/devcenter.yaml`, `devcenter.schema.json`            |

### 🚀 7.5 Deployment Quality Standards

#### 7.5.1 Tiered Definition of Done Standard

Work completion quality is governed by a three-tier Definition of Done
framework: **Task DoD** requires PR merged, validation complete, and
documentation updated; **Feature DoD** requires acceptance criteria met, examples
updated, documentation updated, and validation in target environment; **Epic
DoD** requires all child issues completed, end-to-end scenario validated,
documentation published, and exit metrics met. Each tier builds upon the
previous, ensuring cumulative quality assurance from individual code changes to
complete capability delivery.

| 🏛️ Attribute       | 📝 Value                                                                      |
| ------------------ | ----------------------------------------------------------------------------- |
| **Standard Name**  | Three-Tier Definition of Done                                                 |
| **Enforcement**    | `CONTRIBUTING.md` governance documentation + Issue Form completion checklists  |
| **Scope**          | All work items at Task, Feature, and Epic levels                              |
| **Source**         | `CONTRIBUTING.md` (lines 103–144), `.github/ISSUE_TEMPLATE/*.yml`             |

#### 7.5.2 Deployment Validation Standard

All infrastructure deployments must satisfy minimum validation requirements
before acceptance: `what-if` (or equivalent) validation for all Bicep
deployments, successful deployment in a sandbox or subscription, and documented
basic smoke test steps. Epic-level readiness additionally requires platform
admin flow validation, developer flow validation, networking/DNS assumption
validation, and confirmation that diagnostics are emitting to the expected Log
Analytics sink.

| 🏛️ Attribute       | 📝 Value                                                                     |
| ------------------ | ---------------------------------------------------------------------------- |
| **Standard Name**  | Minimum Deployment Validation Requirements                                   |
| **Enforcement**    | `CONTRIBUTING.md` Feature/Epic DoD criteria                                  |
| **Scope**          | All infrastructure deployments and platform releases                         |
| **Source**         | `CONTRIBUTING.md` (lines 103–121)                                            |

### 📝 7.6 Documentation Standards

#### 7.6.1 Module Documentation Standard

Every Bicep module and automation script must include co-located documentation
covering four mandatory sections: **Purpose** (what the module does),
**Inputs/Outputs** (parameters and return values), **Example Usage** (sample
invocation), and **Troubleshooting** (common failure patterns and resolution
steps). Documentation must be committed in the same pull request as the code
change to maintain synchronization.

| 🏛️ Attribute       | 📝 Value                                                                     |
| ------------------ | ---------------------------------------------------------------------------- |
| **Standard Name**  | Four-Section Module Documentation                                            |
| **Enforcement**    | PR review gate requirement in `CONTRIBUTING.md`                              |
| **Scope**          | All Bicep modules and automation scripts                                     |
| **Source**         | `CONTRIBUTING.md` (lines 92–99)                                              |

#### 7.6.2 Branch and Pull Request Standards

All branches must follow the prefix-based naming convention: `feature/<name>`,
`task/<name>`, `fix/<name>`, or `docs/<name>`, with issue numbers included where
applicable (e.g., `feature/123-dev-center-baseline`). All pull requests must
reference the issue they close (e.g., `Closes #123`), include a summary of
changes, provide test/validation evidence, and include documentation updates
where applicable.

| 🏛️ Attribute       | 📝 Value                                                                     |
| ------------------ | ---------------------------------------------------------------------------- |
| **Standard Name**  | Branch Naming and PR Content Requirements                                    |
| **Enforcement**    | `CONTRIBUTING.md` PR guidelines                                              |
| **Scope**          | All branches and pull requests                                               |
| **Source**         | `CONTRIBUTING.md` (lines 49–71)                                              |
...
| --- | ---------- | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------- | -------- |
| 1   | gap        | No formal alert rule definitions found in source files for Observability Platform capability            | Documented as maturity gap in Section 4 and Section 5.2.4            | Open     |
| 2   | gap        | No end-to-end developer self-service onboarding guide found in repository                              | Documented as maturity gap in Section 5.2.5                          | Open     |
| 3   | gap        | No explicit KPI measurement targets, automated collection pipelines, or dashboard specifications found | Documented as maturity gap in Section 4 and Sections 5.11.1–5.11.4  | Open     |
| 4   | gap        | No formal Security & Compliance Officer RACI matrix found                                              | Documented as maturity gap in Section 5.6 and Section 4              | Open     |
| 5   | gap        | No dedicated `/docs/standards/` directory for formalized architecture standards documentation           | Standards inferred from schema constraints and contribution guidelines | Open     |
| 6   | assumption | Database type and storage strategy inferred from PaaS-only model (no explicit data persistence layer)  | Noted as infrastructure-focused architecture in Section 1            | Resolved |
| 7   | assumption | Business strategy inferred from README, CONTRIBUTING.md, and configuration file patterns               | Documented inference basis in Section 5.1.1                          | Resolved |
| 8   | limitation | Section 6 (Architecture Decisions) excluded per `output_sections` parameter [1, 2, 3, 4, 5, 7, 8]     | Section intentionally omitted as specified in generation parameters   | Resolved |
...
| ------- | ------------------------------- | ------- | ------ |
| G-001   | Dependency Loading              | 100/100 | Pass   |
| G-002   | Workspace Coverage              | 100/100 | Pass   |
| G-003   | Section Completeness            | 100/100 | Pass   |
| G-004   | Source Traceability              | 100/100 | Pass   |
| G-005   | No Fabricated Content           | 100/100 | Pass   |
| G-006   | No Placeholder Text             | 100/100 | Pass   |
| G-007   | Mermaid Diagram Compliance      | 100/100 | Pass   |
| G-008   | TOGAF Classification Compliance | 100/100 | Pass   |
