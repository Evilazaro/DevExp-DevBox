...
This section documents key architectural decision records (ADRs) identified from
source code patterns, configuration choices, and documentation in the
DevExp-DevBox repository. Each decision is traceable to source files and
represents a deliberate architectural choice with documented rationale and
consequences.

The decisions analyzed cover infrastructure organization, security posture,
configuration management, and automation strategy — reflecting a
governance-first approach to platform engineering.

| ADR#    | Decision                                                      | Status   | Date       |
| ------- | ------------------------------------------------------------- | -------- | ---------- |
| ADR-001 | Segregate resources by function into three resource groups    | Accepted | 2026-02-26 |
| ADR-002 | Externalize configuration to YAML with JSON Schema validation | Accepted | 2026-02-26 |
| ADR-003 | Use SystemAssigned managed identities over service principals | Accepted | 2026-02-26 |
| ADR-004 | Restrict deployments to 16 approved Azure regions             | Accepted | 2026-02-26 |

### ADR-001: Segregate Resources by Function

**Context**: Enterprise Dev Box deployments require clear separation of concerns
for RBAC, cost attribution, and lifecycle management across security,
monitoring, and workload resources.

**Decision**: Create three dedicated resource groups (Security, Monitoring,
Workload) aligned with Azure Landing Zone principles.

**Rationale**:

- Enables independent lifecycle management per domain
- Provides granular RBAC scoping (Key Vault access separate from Dev Center
  access)
- Supports clear cost attribution via costCenter tags per resource group

**Consequences**:

- (+) Clear organizational boundaries and cost tracking
- (+) Independent RBAC scoping reduces blast radius
- (-) Increased resource group count adds management overhead
- (-) Cross-resource-group references require subscription-level scoping
...
**Context**: Infrastructure templates need to support multiple deployment
environments (dev, staging, UAT) without code changes to Bicep modules.

**Decision**: Use YAML configuration files with JSON Schema validation, loaded
via Bicep's `loadYamlContent()` function.

**Rationale**:

- Configuration changes require YAML edits, not code changes
- JSON Schema catches errors at design time rather than deployment time
- YAML is human-readable and version-controllable

**Consequences**:

- (+) Repeatable deployments across environments without modifying templates
- (+) Schema validation prevents configuration errors before deployment
- (-) Additional schema maintenance burden
- (-) Bicep `loadYamlContent()` ties configuration to Bicep deployment lifecycle

**Source**: `infra/main.bicep:35`, `infra/settings/workload/devcenter.yaml:1-*`

### ADR-003: Use SystemAssigned Managed Identities

**Context**: Dev Center and project resources need authenticated access to Key
Vault, RBAC APIs, and other Azure services without managing credentials.

**Decision**: Use SystemAssigned managed identities for both Dev Center and
per-project resources, with scoped RBAC role assignments.

**Rationale**:

- Eliminates credential management for service-to-service authentication
- Follows principle of least privilege with scoped role assignments
- Aligns with Microsoft's recommended identity model for Dev Box

**Consequences**:

- (+) No credential rotation required for infrastructure identities
- (+) Identity lifecycle tied to resource lifecycle (auto-cleanup)
- (-) Cannot reuse identity across resources (each gets its own)
- (-) Requires User Access Administrator role for initial assignment

**Source**: `infra/settings/workload/devcenter.yaml:18-22`,
`infra/settings/workload/devcenter.yaml:91-96`

### ADR-004: Restrict Deployments to Approved Regions

**Context**: Enterprise governance requires controlling which Azure regions can
host Dev Box resources to ensure compliance, data residency, and cost control.

**Decision**: Use Bicep `@allowed` decorator to restrict deployments to 16
approved Azure regions.

**Rationale**:

- Ensures compliance with organizational data residency policies
- Prevents accidental deployments to unsupported or high-cost regions
- Enforces at deployment time via parameter validation

**Consequences**:

- (+) Deployment-time enforcement prevents policy violations
- (+) Explicit region list is auditable and version-controlled
- (-) Adding new regions requires Bicep template changes
- (-) May block deployments needed for disaster recovery to unlisted regions
