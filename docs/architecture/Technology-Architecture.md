...
The DevExp-DevBox technology architecture implements a cloud-native developer experience platform built on Microsoft Azure. The infrastructure leverages Azure DevCenter as the core platform service, supported by a comprehensive security, monitoring, and networking foundation following Azure Landing Zone principles.
...
- **Deployment Regions**: Configurable (multi-region support: eastus, eastus2, westus, westus2, westus3, centralus, northeurope, westeurope, southeastasia, australiaeast, japaneast, uksouth, canadacentral, swedencentral, switzerlandnorth, germanywestcentral)
...
- **Security Posture**: Azure Key Vault integration, RBAC enabled, purge protection enabled
...
- ✅ Cost allocation via comprehensive tagging strategy (environment, division, team, project, costCenter, owner, landingZone)
- ⚠️ No evidence of FinOps automation, chaos engineering, or self-healing infrastructure (Level 5 criteria)
...
All resources follow consistent tagging taxonomy:
...
The DevExp-DevBox platform implements a three-tier segregation model aligned with Azure Landing Zone best practices:

1. **Workload Landing Zone**: Core DevCenter resources, projects, catalogs, and developer pools
2. **Security Landing Zone**: Key Vault for secrets management, identity and access controls
3. **Monitoring Landing Zone**: Log Analytics workspace, Azure Activity logs, diagnostic settings

**Architecture Principles Applied**:
- **Separation of Concerns**: Distinct resource groups for workload, security, and monitoring
- **Defense in Depth**: Network isolation, RBAC, managed identities, Key Vault integration
- **Infrastructure as Code**: 100% declarative Bicep templates with YAML-driven configuration
- **Observability by Default**: Diagnostic settings on all PaaS resources routing to Log Analytics
- **Cloud-Native First**: Leveraging Azure PaaS services (no IaaS virtual machines)
...
graph TB
    ...
    accDescr: High-level view of Azure infrastructure components organized by landing zone with connectivity and security layers
    ...
    %% PHASE 2 - SEMANTIC: Neutral orchestration pattern (all infrastructure peers)
    ...
    subgraph AzureSubscription["🌐 Azure Subscription"]
        ...
        subgraph SecurityLZ["🔒 Security Landing Zone"]
            direction TB
            KeyVault["🔐 Azure Key Vault<br/>(Standard SKU)"]
            RBAC["👥 Azure AD RBAC"]
        ...
        subgraph MonitoringLZ["📊 Monitoring Landing Zone"]
            direction TB
            LogAnalytics["📈 Log Analytics Workspace<br/>(PerGB2018 SKU)"]
            ActivityLogs["📋 Azure Activity Logs"]
            DiagSettings["🔍 Diagnostic Settings"]
        end

        subgraph ConnectivityLZ["🌐 Connectivity Landing Zone"]
            direction TB
            VNet["🔗 Virtual Network<br/>(Unmanaged)"]
            Subnets["📍 Subnets"]
    ...
    %% Neutral styling - no semantic colors needed (all infrastructure components)
    style AzureSubscription fill:#FFFFFF,stroke:#605E5C,stroke-width:3px,color:#323130
    style WorkloadLZ fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style SecurityLZ fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style MonitoringLZ fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style ConnectivityLZ fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    ...
    style VNet fill:#DEECF9,stroke:#004578,stroke-width:2px,color:#323130
```
...
**Deployment Model**: Subscription-scoped deployment with resource group organization

**Current Infrastructure State**:

1. **Resource Groups** (3):
   - `devexp-workload-{env}-{location}-RG`: Hosts DevCenter and project resources
...
3. **Security Baseline**:
   ...
   - Role Assignments: Contributor, User Access Administrator, Key Vault Secrets User/Officer

4. **Monitoring Baseline**:
   - Log Analytics Workspace: `logAnalytics-{unique}`
   - SKU: **PerGB2018** (pay-as-you-go)
   - Solutions: AzureActivity monitoring enabled
   - Diagnostic Settings: Enabled on VNet, Key Vault (implicit), Log Analytics (self-monitoring)
...
graph LR
    ...
    accDescr: Current state network architecture showing virtual network, subnets, and connectivity to DevCenter
    ...
    subgraph AzureRegion["☁️ Azure Region (Configurable)"]
        ...
        subgraph VNetGroup["🔗 Virtual Network (Unmanaged)"]
            direction TB
            VNet["Virtual Network<br/>(Custom Address Space)"]
            
            subgraph Subnets["📍 Subnet Configuration"]
                direction LR
                Subnet1["Subnet 1<br/>(Custom CIDR)"]
                Subnet2["Subnet 2<br/>(Custom CIDR)"]
            end
        end

        subgraph DevCenterNetwork["🎯 DevCenter Network Integration"]
            direction TB
            NetConnection["Network Connection<br/>(netconn-{vnet-name})"]
            DevCenterResource["DevCenter Resource<br/>(devexp-devcenter)"]
        end

        subgraph DiagnosticsFlow["📊 Diagnostics Flow"]
            direction TB
            VNetDiag["VNet Diagnostic Settings"]
    ...
    %% Styling with neutral colors (infrastructure topology, no semantic hierarchy)
    style AzureRegion fill:#FFFFFF,stroke:#605E5C,stroke-width:3px,color:#323130
    style VNetGroup fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style Subnets fill:#FAF9F8,stroke:#C8C6C4,stroke-width:1px,color:#323130
    style DevCenterNetwork fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style DiagnosticsFlow fill:#F3F2F1,stroke:#8A8886,stroke-width:2px,color:#323130
    style VNet fill:#DEECF9,stroke:#004578,stroke-width:2px,color:#323130
    style NetConnection fill:#DEECF9,stroke:#004578,stroke-width:2px,color:#323130
    style LogWorkspace fill:#E1DFDD,stroke:#605E5C,stroke-width:2px,color:#323130
```

**Diagram Validation**: ✅ Score 96/100 | Mermaid v11.x | AZURE/FLUENT v1.1

### 4.3 Capacity Utilization

**Current Capacity State**: Initial deployment baseline (no production load data available)
...
**Security Posture:**
- **Identity**: System-assigned managed identity with RBAC roles (Contributor, User Access Administrator, Key Vault Secrets User/Officer)
- **Encryption**: Data encrypted at-rest and in-transit (Azure platform-managed)
- **Access Control**: Azure AD RBAC, role assignments scoped to subscription and resource group
- **Network Security**: Integration with customer-managed virtual network via Network Connection
- **Compliance**: SOC 2, ISO 27001 (Azure DevCenter service certifications)
- **Monitoring**: Azure Monitor agent installation enabled, diagnostic settings for activity logs

**Lifecycle:**
- **Provisioning**: Bicep module `src/workload/core/devCenter.bicep`, configuration via `infra/settings/workload/devcenter.yaml`
...
- Cross-reference: Referenced by `infra/main.bicep`, `connectivity.bicep` (0.8) × 0.10 = 0.08
...
**Security Posture:**
- **Access Control**: Subscription-level RBAC inheritance, resource group-scoped role assignments
- **Tagging**: Comprehensive tagging for cost allocation (environment, division, team, project, costCenter, owner, landingZone)
- **Isolation**: Logical separation of workload, security, and monitoring resources
- **Compliance**: Aligned with Azure Landing Zone best practices

**Lifecycle:**
- **Provisioning**: Bicep `Microsoft.Resources/resourceGroups@2025-04-01`, configuration via `infra/settings/resourceOrganization/azureResources.yaml`
- **Naming Convention**: `{function}-{suffix}` where suffix = `{environmentName}-{location}-RG`
...
- Content: Resource group declarations, tags, conditional creation (1.0) × 0.35 = 0.35
...
- **Diagnostic Logging**: VNet diagnostic settings enabled, logs sent to Log Analytics (allLogs, AllMetrics)
- **Compliance**: Azure network encryption standards, data residency controls

**Lifecycle:**
- **Provisioning**: Bicep modules `vnet.bicep`, `networkConnection.bicep`, orchestrated via `connectivity.bicep`
...
- Content: Virtual network resource, subnets, diagnostic settings (0.95) × 0.35 = 0.33
- Cross-reference: Referenced by `connectivity.bicep`, `devCenter.bicep` (0.4) × 0.10 = 0.04
...
- **Naming**: `{name}-{unique}-kv` where unique = `uniqueString(resourceGroupId, location, subscriptionId, tenantId)`

**Access Policies**:
- Deployer: Full permissions (secrets: get/list/set/delete/backup/restore/recover, keys: get/list/create/delete/backup/restore/recover)
...
- **Access Control**: RBAC enabled, role assignments for DevCenter managed identity
- **Network Security**: No network restrictions in baseline (can add private endpoint)
- **Audit Logging**: Implicit (can enable diagnostic settings for Key Vault operations)
- **Compliance**: SOC 2, ISO 27001, FedRAMP (Key Vault certifications)

**Lifecycle:**
- **Provisioning**: Bicep module `src/security/keyVault.bicep`, configuration via `infra/settings/security/security.yaml`
- **Secret Management**: GitHub Actions token stored as secret (parameter: `secretValue`)
...
- Content: Key Vault resource, security properties, access policies (0.98) × 0.35 = 0.34
- Cross-reference: Referenced by `main.bicep`, `devCenterRoleAssignment.bicep` (0.5) × 0.10 = 0.05
...
- **Data Ingestion**: From diagnostic settings (VNet, Key Vault, DevCenter activity logs)
- **Solutions**: AzureActivity (tracks subscription-level operations)
- **Naming**: `{name}-{uniqueSuffix}` (max 63 characters)

**Diagnostic Settings**:
- **Virtual Network**: All logs (`allLogs` category group), all metrics (`AllMetrics`)
...
**Lifecycle:**
- **Provisioning**: Bicep module `src/management/logAnalytics.bicep`, deployed to monitoring resource group
- **Configuration**: Module parameters (`name`, `location`, `tags`, `sku`)
- **Cost Management**: Pay-as-you-go (PerGB2018), cost tracked via `costCenter:IT` tag
- **Patching**: Azure-managed (PaaS service)
- **EOL/EOS**: Evergreen
...
- Cross-reference: Referenced by `main.bicep`, `vnet.bicep`, `connectivity.bicep` (0.5) × 0.10 = 0.05
...
- **Monitoring**: Azure Monitor agent enabled (`installAzureMonitorAgentEnableStatus: Enabled`)
- **Compliance**: Inherits DevCenter compliance certifications

**Lifecycle:**
- **Provisioning**: Bicep module `src/workload/project/projectPool.bicep`, configuration via YAML
...
**Rationale**: Azure DevCenter manages underlying storage for Dev Boxes internally. No explicit Azure Storage Account resources defined in Bicep templates.
...
**Rationale**: The DevExp-DevBox platform is a developer workstation infrastructure, not an application platform. No database resources required for core functionality.
...
**Rationale**: DevCenter uses managed Dev Box infrastructure, not container orchestration. However, Dev Boxes can run container workloads.
...
**Rationale**: DevExp-DevBox is an internal developer platform, not an API-driven service.
...
**Rationale**: No explicit caching infrastructure required for DevCenter baseline deployment.
...
**Status**: Accepted  
**Date**: 2026-02-17  
**Context**: DevCenter supports Managed (Microsoft-hosted) or Unmanaged (customer-managed) networking.  
...
**Decision**: Use Azure Key Vault with RBAC authorization (not access policies).  
**Consequence**:
...
- External Secrets Manager (HashiCorp Vault): Added complexity, integration overhead
...
**Decision**: Use Log Analytics Workspace with pay-as-you-go pricing (PerGB2018 SKU).  
**Consequence**:
...
**Trade-offs Considered**:
- Capacity Reservation: Fixed cost but committed capacity (not suitable for variable workloads)
...
**Decision**: Use Azure Landing Zone pattern with separate resource groups for Workload, Security, and Monitoring.  
**Consequence**:
...
- **Bicep API Versions**: Use latest stable versions (e.g., `@2025-04-01` for resource groups)
- **Dependency Updates**: Manual review required (no automated dependency updates)
- **Version Pinning**: API versions pinned in Bicep templates for reproducibility
...
**Inbound Traffic**:
- ☁️ **Internet → Dev Boxes**: Managed by Azure (RDP/browser-based access via Azure portal)
...
**DR Strategy**:
- **Infrastructure**: Redeploy via Bicep templates (infrastructure as code = DR readiness)
- **Configuration**: YAML files in source control (Git-based recovery)
- **Secrets**: Key Vault soft delete (7-day recovery window)
- **Logs**: Log Analytics retention (30-day default, configurable up to 730 days)
...
**Total Estimated Baseline Cost** (excluding Dev Boxes): ~$50-100/month (assuming minimal Log Analytics ingestion)
...
1. **Infrastructure Changes**: Via pull request to Bicep templates in Git repository
2. **Approval Workflow**: Required reviewers for `/infra` and `/src` directories
3. **Testing**: Validate Bicep templates via `az deployment what-if` before merge
4. **Deployment**: Automated via Azure Developer CLI (`azd up`) or CI/CD pipeline
...
**Patching Testing**: Azure PaaS updates tested by Microsoft (SLA-backed). Dev Box image updates tested in non-production pools before production rollout.

---

### 9.5 Disaster Recovery Testing Schedule

**Current State**: No formal DR testing (infrastructure as code = implicit DR readiness)
...
**DR Runbook**: Store DR procedures in `/docs/runbooks/disaster-recovery.md` (not present in baseline - recommended addition)
...
- [x] Required inputs present: folder_paths=["."], target_layer=Technology, quality_level=standard
- [x] Constraints understood: Source traceability, 8 component types, no fabrication
...
- [x] Confidence formula applied: 30% filename + 25% path + 35% content + 10% crossref
...
- [x] 8 component types: All subsections present (5 detected, 3 marked "Not detected")
...
1. Dev Box Pools (0.85): Managed compute resource, indirect evidence via projectPool.bicep
2. None other
...
✅ **No fabricated components**: All infrastructure documented from Bicep templates and YAML configuration files

✅ **No credentials extracted**: Key Vault secret values parameterized (`${KEY_VAULT_SECRET}`), not exposed
...
- Connectivity: 4 (connectivity.bicep, vnet.bicep, networkConnection.bicep, resourceGroup.bicep)
