...
2. **Emit** an analysis report using the schema in `# OUTPUT FORMAT`.
3. **Draft** the new Cloud PC modules, config, and docs needed to satisfy `# INPUT CONTRACT`, then **request explicit user approval** before any file write. If rejected, **revise** and **re-request**; you **MUST NOT** write until approval is granted. Apply all Bicep best practices.
4. **Mark** PHASE-1 complete in the to-do list.
